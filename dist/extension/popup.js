...
`)),n.trim()},setLoading:function(e,t="..."){this.isLoading=e,e?(this.pageStatus&&(this.pageStatus.textContent=t),this.setAriaStatus(t)):(this.pageStatus&&!this.pageStatus.textContent.startsWith("\u2705")&&(this.pageStatus.textContent="\u2705 Done"),this.setAriaStatus("Ready"))},updateQuickActionsVisibility:function(){this.quickActions&&this.quickActions.classList.toggle("hidden",!this.pageContent)},resetScreenForGeneration:function(){this.sidebar&&(this.sidebar.classList.add("hidden"),this.sidebar.style.display="none"),this.messagesContainer&&(this.messagesContainer.innerHTML=""),this.updateQuickActionsVisibility()},renderCard:function(e,t,n={}){let a=document.createElement("div");a.className="twitter-content-container";let s=document.createElement("div");s.className="twitter-card analytics-card",s.dataset.contentType=n.contentType||"content",s.dataset.contentId=n.contentId||Date.now().toString();let i={summary:"\u{1F4DD}",keypoints:"\u{1F511}",analysis:"\u{1F4CA}",faq:"\u2753",factcheck:"\u2705",blog:"\u{1F4F0}",proscons:"\u2696\uFE0F",timeline:"\u{1F4C5}",quotes:"\u{1F4AC}"},o=n.contentType||"content",r=i[o]||"\u{1F4C4}",d=n.markdown?`data-markdown="${encodeURIComponent(n.markdown)}"`:"";if(s.innerHTML=`
      ...
      `,window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let m=n.contentType||"content",h={id:n.contentId||Date.now().toString(),content:n.markdown||t,title:e},w=s.querySelector(".twitter-header-actions");w&&window.TabTalkUI.addSaveButtonToCard(w,m,h)}let u=s.querySelector(".copy-btn"),c=u.innerHTML;u.addEventListener("click",async m=>{m.stopPropagation();try{let h=s.querySelector(".structured-html"),w=h?.getAttribute("data-markdown"),g=w?decodeURIComponent(w):h?.innerText||"";await navigator.clipboard.writeText(g),u.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
Craft your thread now:`}let m=await this.callGeminiAPIWithSystemPrompt(r,d);if(m){let h=this.cleanTwitterContent(m);if(n==="thread"){let y=this.parseTwitterThread(h)[0]||h;s.value=y}else s.value=h;let w=e.querySelector(".twitter-char-count"),g=this.getAccurateCharacterCount(s.value);w.textContent=`${g} characters`,setTimeout(()=>{s.style.height="auto",s.style.height=Math.max(80,s.scrollHeight)+"px"},0)}}catch(r){console.error("Error regenerating content:",r),alert("Error regenerating content. Please try again.")}finally{i.textContent="\u{1F504}",i.disabled=!1}},getDefaultToneInstructions:function(e){let t={supportive:`TONE: Supportive with Facts
...
Generate the complete thread now:`,h=await a.callGeminiAPIWithSystemPrompt(l,m);if(!h)throw new Error("Failed to expand thread");console.log("\u2705 Thread expanded");let w=a.cleanTwitterContent(h),g=a.parseTwitterThread(w),v=[];for(let f of g)if(a.getAccurateCharacterCount(f)<=280)v.push(f);else{let C=await p.smartSplitTweet.call(a,f,280);v.push(...C)}console.log(`\u2705 Thread generated: ${v.length} tweets`);let y=`thread_${Date.now()}`;p.renderThreadGeneratorResult.call(a,v,y,e,t,s),a.autoSaveThread&&await a.autoSaveThread(y,v,w),await a.saveState()}catch(r){console.error("Error generating thread:",r),alert(`\u274C Error generating thread: ${r.message}`)}finally{a.setLoading(!1),a.hideProgressBar&&a.hideProgressBar()}},smartSplitTweet:async function(e,t){let n=e.match(/[^.!?]+[.!?]+/g)||[e],a=[],s="";for(let i of n)this.getAccurateCharacterCount(s+i)<=t?s+=i:(s&&a.push(s.trim()),s=i);return s&&a.push(s.trim()),a.length>0?a:[e.substring(0,t)]},renderThreadGeneratorResult:function(e,t,n,a,s=!0){let i=document.createElement("div");i.className="twitter-content-container thread-generator-result",i.dataset.category=n,i.dataset.topic=a,i.dataset.useKnowledgePack=s;let o=document.createElement("div");o.className="thread-header";let r=this.getTotalChars(e);o.innerHTML=`
        <div class="thread-info">
          <span class="thread-icon">\u{1F9F5}</span>
      ...
      `,i.appendChild(c);let l=c.querySelector(".master-length-slider"),m=c.querySelector(".current-length"),h=c.querySelector(".btn-regenerate-thread"),w=c.querySelectorAll(".preset-btn");l.addEventListener("input",g=>{m.textContent=g.target.value}),w.forEach(g=>{g.addEventListener("click",()=>{let v=g.dataset.length;l.value=v,m.textContent=v})}),h.addEventListener("click",async()=>{let g=parseInt(l.value);await this.regenerateEntireThreadForGenerator(i,t,g,n,a,s)}),e.forEach((g,v)=>{let y=`Thread ${v+1}/${e.length}`,f=this.createTwitterCard(g,y,!0);f.dataset.platform="thread",f.dataset.threadId=t,f.dataset.category=n,i.appendChild(f)}),this.messagesContainer.appendChild(i),setTimeout(()=>{this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"})},100)},regenerateEntireThreadForGenerator:async function(e,t,n,a,s,i){let o=e.querySelector(".btn-regenerate-thread");if(!o)return;let r=o.textContent;o.textContent="\u23F3 Regenerating...",o.disabled=!0;try{let d=Math.max(3,Math.min(12,Math.ceil(n/400))),u="";if(i){let h=await this.loadKnowledgePack(a);h&&h.facts&&(u=`

RELEVANT KNOWLEDGE BASE:
${h.facts.slice(0,5).map((w,g)=>`${g+1}. ${w}`).join(`
`)}
...
- Keep it fun but factual`,keywords:["playful","humorous","sassy","internet-culture","evidence-backed"]},"anti-propaganda":{id:"anti-propaganda",name:"Anti-Propaganda",icon:"\u{1F6E1}\uFE0F",color:"#8b5cf6",category:"investigative",description:"Debunk myths with facts & humor",example:"The claim vs. the reality...",aiInstructions:`TONE: Anti-Propaganda
...
- Maintain credibility while being entertaining`,keywords:["fact-checking","debunking","truth-seeking","investigative","educational"]},"critical-humor":{id:"critical-humor",name:"Critical with Humor",icon:"\u{1F605}",color:"#f97316",category:"playful",description:"Clever critique with witty observations",example:"This is like that time when... but actually...",aiInstructions:`TONE: Critical with Humor
...
- Keep it light but insightful`,keywords:["witty","clever","analogies","entertaining","insightful"]},sarcastic:{id:"sarcastic",name:"Sarcastic",icon:"\u{1F3AD}",color:"#ec4899",category:"playful",description:"Ironic commentary with bite",example:"Oh sure, because that worked so well before...",aiInstructions:`TONE: Sarcastic
...
- Make the irony obvious but sophisticated`,keywords:["ironic","sarcastic","rhetorical","clever","pointed"]},investigative:{id:"investigative",name:"Investigative",icon:"\u{1F50D}",color:"#06b6d4",category:"analytical",description:"Deep dive analysis with evidence",example:"Digging deeper reveals...",aiInstructions:`TONE: Investigative
...
- Balance enthusiasm with realism`,keywords:["hopeful","inspiring","future-focused","positive","motivating"]},cautionary:{id:"cautionary",name:"Cautionary",icon:"\u26A0\uFE0F",color:"#eab308",category:"analytical",description:"Warn about risks and considerations",example:"Before you proceed, consider this...",aiInstructions:`TONE: Cautionary
...
- Back warnings with evidence`,keywords:["warning","risk-aware","careful","balanced","informative"]},empowering:{id:"empowering",name:"Empowering",icon:"\u{1F4AA}",color:"#a855f7",category:"positive",description:"Inspire action and personal agency",example:"You have the power to change this by...",aiInstructions:`TONE: Empowering
