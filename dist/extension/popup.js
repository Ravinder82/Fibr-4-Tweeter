...
${a.researchContext}`,s=`${e.id==="repurpose"?"REPHRASE this content with better wording - DO NOT add your own opinions or change the message.":"Share your authentic thoughts about this content - exactly like you'd tweet it to your followers."}
...
Produce the final comment now in plain text only. Fresh run ID: ${Date.now()}`,s=await this.callGeminiAPIWithSystemPrompt(a,i);if(!s)throw new Error("Empty response received from Gemini API");let o=this.cleanTwitterContent(s);if(this.addTwitterMessage("assistant",o,"comment"),this.addToHistory){let r={timestamp:new Date().toISOString(),url:this.currentTab?.url||"",title:this.currentTab?.title||"",domain:this.currentDomain||"",content:o,type:"comment"};await this.addToHistory("comment",r)}await this.saveState()}catch(e){console.error("Error generating comment reply:",e),console.error("Error details:",{message:e.message,stack:e.stack,hasApiKey:!!this.apiKey,hasPageContent:!!this.pageContent,toneId:t?.id}),this.showToast?this.showToast(`\u274C Comment reply failed: ${e.message}`,4e3):alert(`\u274C Comment reply failed: ${e.message}`)}finally{this.setLoading(!1),this.hideProgressBar()}},showProgressBar:function(t){this.hideProgressBar();let e=document.createElement("div");e.className="progress-container",e.id="twitter-progress",e.innerHTML=`
        <div class="progress-message">${t}</div>
        <div class="progress-bar">
          <div class="progress-fill"></div>
        </div>
      `,this.messagesContainer.appendChild(e),this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"}),setTimeout(()=>{let n=e.querySelector(".progress-fill");n&&(n.style.width="100%")},100)},hideProgressBar:function(){let t=document.getElementById("twitter-progress");t&&t.remove()},addTwitterMessage:function(t,e,n,a=null){this.renderTwitterContent(e,n,a)},renderTwitterContent:function(t,e,n=null){let a=document.createElement("div");if(a.className="twitter-content-container",e==="thread"){let i=this.parseTwitterThread(t);i.length<=1&&t.includes("1/")&&(console.warn("\u26A0\uFE0F  Thread parsing may have failed - got single tweet but content suggests thread"),console.log("Original content length:",t.length),console.log("Parsed tweets count:",i.length));let s=`thread_${Date.now()}`;this.autoSaveThread(s,i,t);let o=document.createElement("div");o.className="thread-header";let r=this.getTotalChars(i);o.innerHTML=`
...
${h.researchContext}`,l=`Share your genuine thoughts about this content in ${e} characters - like you're talking to friends.
...
Share your authentic thread now:`}let m=await this.callGeminiAPIWithSystemPrompt(r,l);if(m){let p=this.cleanTwitterContent(m);if(n==="thread"){let T=this.parseTwitterThread(p)[0]||p;i.value=T}else i.value=p;let w=t.querySelector(".twitter-char-count"),u=this.getAccurateCharacterCount(i.value);w.textContent=`${u} characters`,setTimeout(()=>{i.style.height="auto",i.style.height=Math.max(80,i.scrollHeight)+"px"},0)}}catch(r){console.error("Error regenerating content:",r),alert("Error regenerating content. Please try again.")}finally{s.textContent="\u{1F504}",s.disabled=!1}},getDefaultToneInstructions:function(t){let e={"fact-check":`TONE: Fact Check
...
Generate your expert research thread now:`,c=await this.callGeminiAPIWithSystemPrompt(r,l);if(c){let g=this.cleanTwitterContent(c),h=this.parseTwitterThread(g);t.querySelectorAll(".twitter-card").forEach(y=>y.remove()),h.forEach((y,T)=>{let f=`Thread ${T+1}/${h.length}`,v=this.createTwitterCard(y,f,!0);v.dataset.platform="thread",v.dataset.threadId=e,t.appendChild(v)});let p=t.querySelector(".thread-meta");p&&(p.textContent=`${h.length} tweets \u2022 ${this.getTotalChars(h)} chars`);let w=t.querySelector(".current-length");w&&(w.textContent=this.getTotalChars(h));let u=t.querySelector(".master-length-slider");u&&(u.value=this.getTotalChars(h)),await this.autoSaveThread(e,h,g),console.log("\u2705 Thread regenerated successfully")}}catch(o){console.error("Error regenerating thread:",o),alert("Failed to regenerate thread. Please try again.")}finally{i.textContent=s,i.disabled=!1}},escapeHtml:function(t){let e=document.createElement("div");return e.textContent=t,e.innerHTML}};window.TabTalkTwitter=d})();(function(){let d={selectedTone:null,appInstance:null,init:function(){this.createModalEvents(),this.populateReplyTones()},showWithContentLoading:async function(t){if(this.appInstance=t,!t.pageContent||!t.apiKey)if(t.apiKey)await t.getAndCachePageContent();else{this.showToast("\u274C Please set up your Gemini API key first.",3e3);return}this.showModal()},createModalEvents:function(){let t=document.querySelector(".repost-modal-close"),e=document.querySelector("#repost-modal .tone-modal-overlay"),n=document.getElementById("repost-cancel-btn");t?.addEventListener("click",()=>this.hideModal()),e?.addEventListener("click",()=>this.hideModal()),n?.addEventListener("click",()=>this.hideModal()),document.getElementById("repost-generate-btn")?.addEventListener("click",()=>this.handleGenerate()),document.addEventListener("keydown",i=>{i.key==="Escape"&&!document.getElementById("repost-modal").classList.contains("hidden")&&this.hideModal()})},populateReplyTones:function(){let t=document.querySelector("#repost-modal .tone-grid");if(!t||!window.TabTalkToneSelector)return;let e=Object.values(window.TabTalkToneSelector.toneDefinitions).filter(a=>a.category==="reply"&&a.id!=="fact-check");t.innerHTML=e.map(a=>`
...
- Keep it to 2\u20134 sentences total and end with the question itself\u2014no extra fluff or CTA.`}],t={selectedTone:null,appInstance:null,init:function(){this.createModalEvents(),this.populateCommentTones()},showWithContentLoading:async function(e){if(this.appInstance=e,!e.pageContent||!e.apiKey)if(e.apiKey)await e.getAndCachePageContent();else{this.showToast("\u274C Please set up your Gemini API key first.",3e3);return}this.showModal()},createModalEvents:function(){let e=document.querySelector(".comments-modal-close"),n=document.querySelector("#comments-modal .tone-modal-overlay"),a=document.getElementById("comments-cancel-btn"),i=document.getElementById("comments-generate-btn");e?.addEventListener("click",()=>this.hideModal()),n?.addEventListener("click",()=>this.hideModal()),a?.addEventListener("click",()=>this.hideModal()),i?.addEventListener("click",()=>this.handleGenerate()),document.addEventListener("keydown",s=>{let o=document.getElementById("comments-modal");s.key==="Escape"&&o&&!o.classList.contains("hidden")&&this.hideModal()})},populateCommentTones:function(){let e=document.querySelector("#comments-modal .tone-grid");if(!e)return;e.innerHTML=d.map(a=>`
      ...
      `).join(""),e.querySelectorAll(".comments-tone-option").forEach(a=>{a.addEventListener("click",()=>this.selectTone(a)),a.addEventListener("keydown",i=>{(i.key==="Enter"||i.key===" ")&&(i.preventDefault(),this.selectTone(a))})})},showModal:function(){let e=document.getElementById("comments-modal");if(!e)return;e.classList.remove("hidden"),e.setAttribute("aria-hidden","false"),e.querySelector(".comments-tone-option")?.focus()},hideModal:function(){let e=document.getElementById("comments-modal");e&&(e.classList.add("hidden"),e.setAttribute("aria-hidden","true"),this.resetSelections())},selectTone:function(e){document.querySelectorAll(".comments-tone-option").forEach(s=>{s.classList.remove("selected"),s.setAttribute("aria-checked","false")}),e.classList.add("selected"),e.setAttribute("aria-checked","true");let a=e.dataset.toneId;this.selectedTone=d.find(s=>s.id===a)||null;let i=document.getElementById("comments-generate-btn");i&&(i.disabled=!this.selectedTone)},resetSelections:function(){this.selectedTone=null,document.querySelectorAll(".comments-tone-option").forEach(a=>{a.classList.remove("selected"),a.setAttribute("aria-checked","false")});let n=document.getElementById("comments-generate-btn");n&&(n.disabled=!0)},handleGenerate:async function(){if(!this.selectedTone){this.showToast("\u274C Please select a tone first.",2e3);return}if(!this.appInstance){this.showToast("\u274C App not initialized.",3e3);return}let e=this.selectedTone;this.hideModal(),window.TabTalkTwitter&&window.TabTalkTwitter.clearPreviousCommentOutputs&&window.TabTalkTwitter.clearPreviousCommentOutputs.call(this.appInstance);try{if(window.TabTalkTwitter&&typeof window.TabTalkTwitter.generateCommentReplyWithTone=="function")await window.TabTalkTwitter.generateCommentReplyWithTone.call(this.appInstance,e);else if(typeof this.appInstance.generateCommentReplyWithTone=="function")await this.appInstance.generateCommentReplyWithTone(e);else throw new Error("Comment reply generator not available")}catch(n){console.error("TabTalk AI: Failed to generate comment reply",n),this.showToast(`\u274C Comment generation failed: ${n.message}`,4e3)}},showToast:function(e,n=3e3){window.TabTalkUI?.showToast?window.TabTalkUI.showToast(e,n):console.log("Toast:",e)}};window.TabTalkCommentsModal=t,document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>t.init()):t.init()})();(function(){let d={knowledgePacks:{},modalInitialized:!1,popupInstance:null,init:function(){this.modalInitialized||(this.createModalHTML(),this.bindModalEvents(),this.modalInitialized=!0)},createModalHTML:function(){document.getElementById("thread-generator-modal")||document.body.insertAdjacentHTML("beforeend",`
...
`),n.showProgressBar&&n.showProgressBar("Generating thread...");let r="You are a precise thread outline creator. You create structured outlines for engaging Twitter/X threads. No markdown, no hashtags.",l=`Create a ${i}-tweet thread outline about: ${t}
...
Generate the complete thread now:`,m=await n.callGeminiAPIWithSystemPrompt(g,h);if(!m)throw new Error("Failed to expand thread");console.log("\u2705 Thread expanded");let p=n.cleanTwitterContent(m),w=n.parseTwitterThread(p),u=[];for(let T of w)if(n.getAccurateCharacterCount(T)<=280)u.push(T);else{let v=await d.smartSplitTweet.call(n,T,280);u.push(...v)}console.log(`\u2705 Thread generated: ${u.length} tweets`);let y=`thread_${Date.now()}`;d.renderThreadGeneratorResult.call(n,u,y,t,a),n.autoSaveThread&&await n.autoSaveThread(y,u,p),await n.saveState()}catch(o){console.error("Error generating thread:",o),alert(`\u274C Error generating thread: ${o.message}`)}finally{n.setLoading(!1),n.hideProgressBar&&n.hideProgressBar()}},smartSplitTweet:async function(t,e){let n=t.match(/[^.!?]+[.!?]+/g)||[t],a=[],i="";for(let s of n)this.getAccurateCharacterCount(i+s)<=e?i+=s:(i&&a.push(i.trim()),i=s);return i&&a.push(i.trim()),a.length>0?a:[t.substring(0,e)]},renderThreadGeneratorResult:function(t,e,n,a=!0){let i=document.createElement("div");i.className="twitter-content-container thread-generator-result",i.dataset.topic=n,i.dataset.useKnowledgePack=a;let s=document.createElement("div");s.className="thread-header";let o=this.getTotalChars(t);s.innerHTML=`
        <div class="thread-info">
          <span class="thread-icon">\u{1F9F5}</span>
          ...
          <span class="thread-meta">${t.length} tweets \u2022 ${o} chars</span>
        </div>
        <div class="thread-actions">
      ...
      `,i.appendChild(s);let r=s.querySelector(".btn-copy-all-thread"),l=s.querySelector(".copy-all-status");r.addEventListener("click",async()=>{await this.copyAllTweets(t,r,l,e)});let c=document.createElement("div");c.className="thread-master-control",c.innerHTML=`
        <div class="master-control-header">
          <span class="control-label">Thread Length Control</span>
          <span class="control-hint">Adjust total thread length \u2022 Characters distributed proportionally</span>
        </div>
        <div class="master-control-slider">
          ...
          <div class="slider-container">
            <span class="slider-min">500</span>
            <input type="range" class="master-length-slider" min="500" max="5000" value="${o}" step="100" data-thread-id="${e}">
            <span class="slider-max">5000</span>
          </div>
          <div class="slider-value">
...
Generate your expert research thread now:`,g=await this.callGeminiAPIWithSystemPrompt(l,c);if(g){let h=this.cleanTwitterContent(g),m=this.parseTwitterThread(h);this.displayThreadResult(h,n,category),this.saveThreadToGallery(h,n,category),window.TabTalkUI?.showToast("Expert thread generated successfully!",2e3)}}catch(i){console.error("Thread generation failed:",i),window.TabTalkUI?.showToast("Failed to generate thread",3e3)}finally{let i=document.getElementById("generate-thread-btn");i&&(i.textContent="\u{1F680} Generate Enhanced Thread",i.disabled=!1)}}};window.TabTalkThreadGenerator=d})();(function(){let d={initializeHorizontalScroll:function(){let t=document.querySelector(".scroll-container"),e=document.getElementById("scroll-left"),n=document.getElementById("scroll-right");if(!t||!e||!n)return;let a=200;e.addEventListener("click",()=>{t.scrollBy({left:-a,behavior:"smooth"})}),n.addEventListener("click",()=>{t.scrollBy({left:a,behavior:"smooth"})});let i=()=>{let l=t.scrollWidth-t.clientWidth;e.disabled=t.scrollLeft<=0,n.disabled=t.scrollLeft>=l};t.addEventListener("scroll",i),i(),t.addEventListener("wheel",l=>{l.deltaY!==0&&(l.preventDefault(),t.scrollLeft+=l.deltaY,i())});let s=!1,o,r;t.addEventListener("mousedown",l=>{s=!0,o=l.pageX-t.offsetLeft,r=t.scrollLeft,t.style.cursor="grabbing"}),t.addEventListener("mouseleave",()=>{s=!1,t.style.cursor="grab"}),t.addEventListener("mouseup",()=>{s=!1,t.style.cursor="grab",i()}),t.addEventListener("mousemove",l=>{if(!s)return;l.preventDefault();let g=(l.pageX-t.offsetLeft-o)*1.5;t.scrollLeft=r-g}),t.style.cursor="grab"}};window.TabTalkScroll=d})();(function(){let d={INIT_KEY:"savedContent",async loadSaved(t="twitter"){if(!window.TabTalkStorage||!TabTalkStorage.getSavedContent)return console.error("Gallery: TabTalkStorage not available"),[];let e=await TabTalkStorage.getSavedContent();return e?t==="all"?Object.entries(e).flatMap(([a,i])=>Array.isArray(i)?i.map(s=>({...s,_category:a})):[]):Array.isArray(e[t])?e[t]:[]:[]},async render(t,e="twitter"){t.innerHTML="";let n=document.createElement("div");n.className="gallery-header",n.innerHTML=`
      ...
      `;let o=e.querySelector(".gallery-preview"),r=e.querySelector(".btn-action.copy"),l=e.querySelector(".btn-action.read"),c=e.querySelector(".btn-action.edit"),g=e.querySelector(".btn-action.delete");return r.addEventListener("click",async h=>{h.stopPropagation();try{let m="";(window.TabTalkTwitter&&window.TabTalkTwitter.isThreadContent?window.TabTalkTwitter.isThreadContent(t):this.fallbackThreadDetection(t))?m=this.extractThreadContent(t):m=t.content||"",await navigator.clipboard.writeText(m);let w=r.querySelector("span");w.textContent="\u2713",r.classList.add("success"),setTimeout(()=>{w.textContent="Copy",r.classList.remove("success")},1500)}catch(m){console.error("Gallery copy failed",m)}}),l.addEventListener("click",h=>{h.stopPropagation(),this.openReadModal(t)}),c.addEventListener("click",h=>{h.stopPropagation(),this.openEditModal(t)}),g.addEventListener("click",async h=>{h.stopPropagation(),confirm("Delete this saved item?")&&(await this.deleteItem(t),e.remove())}),e.addEventListener("click",h=>{h.target.closest(".btn-action")||this.openReadModal(t)}),e},openReadModal(t){let e=document.createElement("div");e.className="gallery-modal";let n="";e.innerHTML=`
      ...
      `,document.body.appendChild(e);let a=()=>e.remove();e.querySelector(".modal-close").addEventListener("click",a),e.querySelector(".gallery-modal-overlay").addEventListener("click",a),e.querySelector(".modal-btn.copy").addEventListener("click",async()=>{let s="";(t.platform||"").toLowerCase()==="thread"&&Array.isArray(t.tweets)&&t.tweets.length>0?s=t.tweets.map((l,c)=>`${l.number||`${c+1}/${t.tweets.length}:`}
${l.content||""}`).join(`

---

`):s=t.content||"",await navigator.clipboard.writeText(s);let r=e.querySelector(".modal-btn.copy");r.textContent="Copied!",setTimeout(()=>r.textContent="Copy",1500)}),e.querySelector(".modal-btn.edit").addEventListener("click",()=>{a(),this.openEditModal(t)});let i=s=>{s.key==="Escape"&&(a(),document.removeEventListener("keydown",i))};document.addEventListener("keydown",i)},openEditModal(t){let e=document.createElement("div");e.className="gallery-modal",e.innerHTML=`
        <div class="gallery-modal-overlay"></div>
        <div class="gallery-modal-content">
          <div class="gallery-modal-header">
            <div>
