...
`+w),await navigator.clipboard.writeText(h),c.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          </svg>`,c.classList.add("success"),setTimeout(()=>{c.innerHTML=g,c.classList.remove("success")},2e3)}catch(u){console.error("Copy failed:",u)}}),a.appendChild(i);let p=n.container||this.messagesContainer||document.getElementById("messages-container");return p&&(p.appendChild(a),p===this.messagesContainer&&p.scrollTo({top:p.scrollHeight,behavior:"smooth"})),this.updateEmptyState(),i},showProgressBar:function(t){this.hideProgressBar();let e=document.createElement("div");e.className="progress-container",e.id="global-progress",e.innerHTML=`
        <div class="progress-message">${t}</div>
        <div class="progress-bar"><div class="progress-fill"></div></div>
      `,this.messagesContainer.appendChild(e),this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"}),setTimeout(()=>{let n=e.querySelector(".progress-fill");n&&(n.style.width="100%")},100),this.updateEmptyState()},hideProgressBar:function(){let t=document.getElementById("global-progress");t&&t.remove(),this.updateEmptyState()},addSaveButtonToCard:function(t,e,n,a){if(!t||!n||!a)return;let i=document.createElement("button");if(e&&e.classList.contains("twitter-header-actions")?(i.className="twitter-action-btn save-btn",i.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
...
Produce your final comment now in plain text only. Fresh run ID: ${Date.now()}`,o=await this.callGeminiAPIWithSystemPrompt(a,i);if(!o)throw new Error("Empty response received from Gemini API");let r=this.cleanTwitterContent(o);if(this.addTwitterMessage("assistant",r,"comment"),this.addToHistory){let s={timestamp:new Date().toISOString(),url:this.currentTab?.url||"",title:this.currentTab?.title||"",domain:this.currentDomain||"",content:r,type:"comment"};await this.addToHistory("comment",s)}await this.saveState()}catch(e){console.error("Error generating comment reply:",e),console.error("Error details:",{message:e.message,stack:e.stack,hasApiKey:!!this.apiKey,hasPageContent:!!this.pageContent,toneId:t?.id}),this.showToast?this.showToast(`\u274C Comment reply failed: ${e.message}`,4e3):alert(`\u274C Comment reply failed: ${e.message}`)}finally{this.setLoading(!1),this.hideProgressBar()}},showProgressBar:function(t){this.hideProgressBar();let e=document.createElement("div");e.className="progress-container",e.id="twitter-progress",e.innerHTML=`
      ...
      `,this.messagesContainer.appendChild(e),this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"}),setTimeout(()=>{let n=e.querySelector(".progress-fill");n&&(n.style.width="100%")},100)},hideProgressBar:function(){let t=document.getElementById("twitter-progress");t&&t.remove()},addTwitterMessage:function(t,e,n,a=null){this.renderTwitterContent(e,n,a)},renderTwitterContent:function(t,e,n=null){let a=document.createElement("div");if(a.className="twitter-content-container",e==="twitter"?(a.dataset.generationType="repost",a.dataset.generationTimestamp=Date.now().toString()):e==="thread"?a.dataset.generationType="thread":e==="comment"&&(a.dataset.generationType="comment"),e==="thread"){let i=this.parseTwitterThread(t);i.length<=1&&t.includes("1/")&&(console.warn("\u26A0\uFE0F  Thread parsing may have failed - got single tweet but content suggests thread"),console.log("Original content length:",t.length),console.log("Parsed tweets count:",i.length));let o=`thread_${Date.now()}`,r=this.getTotalChars(i),s=document.createElement("div");s.className="thread-header",s.innerHTML=`
        ...
        `,a.appendChild(s);let l=s.querySelector(".btn-copy-all-thread");l.addEventListener("click",async()=>{await this.copyAllTweets(i,l,o)});let c=s.querySelector(".btn-save-all-thread");c.addEventListener("click",async()=>{await this.saveAllTweets(i,c,o,t)});let g=document.createElement("div");g.className="thread-master-control",g.innerHTML=`
...
`+i:a=i;return a.trim()&&e.push(a.trim()),e.filter(i=>i.length>0).map(i=>i.replace(/^\d+\/\d+[\s:]*/,"").trim())},tryFlexiblePatternParsing:function(t){let e=[],n=[/(?:^|\n)(\d+\/\d+)\s*[:\n]\s*([^]*?)(?=\n\d+\/\d+|\n*$)/g,/(?:^|\n)(\d+\/\d+)\s*\n\s*([^]*?)(?=\n\d+\/\d+|\n*$)/g,/(?:^|\n)(\d+)\/(\d+)\s*[:\n]\s*([^]*?)(?=\n\d+\/\d+|\n*$)/g];for(let a of n){let i;for(e.length=0;(i=a.exec(t))!==null;){let o=i[2]||i[1]||"";o.trim()&&e.push(o.trim())}if(e.length>1)break}return e.filter(a=>a.length>0).map(a=>a.replace(/^\d+\/\d+[\s:]*/,"").trim())},tryContentBasedSplitting:function(t){let e=[],n=t.includes("\u{1F9F5}")||t.toLowerCase().includes("thread")||t.length>500,a=t.split(/\n\s*\n|\n---\n/).filter(r=>r.trim());if(a.length>1&&n)for(let r of a){let s=r.trim();s.length>15&&!s.match(/^🧵\s*thread\s*on\s*.*$/i)&&!s.match(/^\d+\.\s*$/)&&e.push(s)}if(e.length<=1&&t.length>600){let r=t.match(/[^.!?]+[.!?]+/g)||[t],s="";for(let l of r)this.getAccurateCharacterCount(s+l)<=280?s+=l:(s.trim()&&e.push(s.trim()),s=l);s.trim()&&e.push(s.trim())}let i=e.filter(r=>{let s=r.trim();return s.length>20&&!s.match(/^🧵\s*thread\s*on\s*.*$/i)&&!s.match(/^\d+\.\s*$/)});return i.length<2&&a.length<=2?[t.trim()]:(i.length>0?i:[t.trim()]).map(r=>r.replace(/^\d+\/\d+[\s:]*/,"").trim())},createTwitterCard:function(t,e,n=!1,a=null){let i=document.createElement("div");i.className="twitter-card";let o=this.currentSelectedTone?`
      ...
      `,window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let m={id:Date.now().toString(),content:t,title:e},u=e.toLowerCase().includes("thread")?"thread":"twitter",y=i.querySelector(".twitter-header-actions");y&&window.TabTalkUI.addSaveButtonToCard(i,y,u,m)}let l=i.querySelector(".copy-btn"),c=i.querySelector(".twitter-text"),g=l.innerHTML;l.addEventListener("click",async m=>{m.stopPropagation();try{let u=c.value,y=i.dataset.imagePrompt?decodeURIComponent(i.dataset.imagePrompt):null,h=a||y;h&&(u+=`
...
Share your authentic tweet now: Generation ID: [timestamp]`;else if(n==="thread"){let u=Math.ceil(e/400);s=`You are an authentic human storyteller on Twitter/X who writes threads exactly like real people talk. Your threads feel like you're sharing a fascinating story or journey with friends in a group chat\u2014natural, conversational, and genuinely engaging. Each tweet builds on the last one naturally, like thinking out loud together.
...
`);await navigator.clipboard.writeText(i);let o=e.innerHTML;e.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="20,6 9,17 4,12"></polyline>
        </svg>`,e.classList.add("success"),this.showToast&&this.showToast("All tweets copied to clipboard!"),setTimeout(()=>{e.innerHTML=o,e.classList.remove("success")},2e3),console.log("\u2705 All tweets (with prompts if available) copied to clipboard")}catch(a){console.error("Error copying all tweets:",a),this.showToast&&this.showToast("Failed to copy tweets")}},saveAllTweets:async function(t,e,n,a){if(!window.FibrStorage){this.showToast&&this.showToast("Gallery storage not available");return}try{let i=e.innerHTML,o=t.join(`

`),r={id:n,content:o,metadata:{source:this.currentTab?.url||window.location.href,title:this.currentTab?.title||"Thread",tweetCount:t.length},type:"thread",platform:"thread",title:"Thread from Page"};await window.FibrStorage.saveContent("twitter",r),e.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="20,6 9,17 4,12"></polyline>
        </svg>`,e.classList.add("success"),this.showToast&&this.showToast("Thread saved to gallery!"),setTimeout(()=>{e.innerHTML=i,e.classList.remove("success")},2e3)}catch(i){console.error("Failed to save thread to gallery:",i),this.showToast&&this.showToast("Failed to save thread")}},getTotalChars:function(t){return t.reduce((e,n)=>e+this.getAccurateCharacterCount(n),0)},showAutoSaveNotification:function(){let t=document.createElement("div");t.className="auto-save-notification",t.innerHTML="\u{1F4BE} Thread auto-saved",t.style.cssText=`
        position: fixed;
        bottom: 20px;
        right: 20px;
...
`),n.showProgressBar&&n.showProgressBar("Generating thread...");let s="You are a precise thread outline creator. You create structured outlines for engaging Twitter/X threads. No markdown, no hashtags.",l=`Create a ${i}-tweet thread outline about: ${t}
...
- Every thread should feel researched, trustworthy, and thrilling on every topic, no matter how niche or broad.`,p=`Transform this outline into a complete ${i}-tweet thread about: ${t}
...
Generate the complete thread now:`,m=await n.callGeminiAPIWithSystemPrompt(g,p);if(!m)throw new Error("Failed to expand thread");console.log("\u2705 Thread expanded");let u=n.cleanTwitterContent(m),y=n.parseTwitterThread(u),h=[];for(let v of y)if(n.getAccurateCharacterCount(v)<=280)h.push(v);else{let T=await d.smartSplitTweet.call(n,v,280);h.push(...T)}console.log(`\u2705 Thread generated: ${h.length} tweets`);let w=`thread_${Date.now()}`;d.renderThreadGeneratorResult.call(n,h,w,t,a),await n.saveState()}catch(r){console.error("Error generating thread:",r),alert(`\u274C Error generating thread: ${r.message}`)}finally{n.setLoading(!1),n.hideProgressBar&&n.hideProgressBar()}},smartSplitTweet:async function(t,e){let n=t.match(/[^.!?]+[.!?]+/g)||[t],a=[],i="";for(let o of n)this.getAccurateCharacterCount(i+o)<=e?i+=o:(i&&a.push(i.trim()),i=o);return i&&a.push(i.trim()),a.length>0?a:[t.substring(0,e)]},renderThreadGeneratorResult:function(t,e,n,a=!0){let i=document.createElement("div");i.className="twitter-content-container thread-generator-result",i.dataset.topic=n,i.dataset.useKnowledgePack=a;let o=document.createElement("div");o.className="thread-header";let r=this.getTotalChars(t);o.innerHTML=`
      ...
      `,i.appendChild(o);let s=o.querySelector(".btn-copy-all-thread");s.addEventListener("click",async()=>{await this.copyAllTweets(t,s,e)});let l=o.querySelector(".btn-save-all-thread");l.addEventListener("click",async()=>{await this.saveAllTweets(t,l,e,n)});let c=document.createElement("div");c.className="thread-master-control",c.innerHTML=`
        <div class="master-control-header">
          <span class="control-label">Thread Length Control</span>
          <span class="control-hint">Adjust total thread length \u2022 Characters distributed proportionally</span>
        </div>
...
Generate your unforgettable thread now:`,p=await this.callGeminiAPIWithSystemPrompt(c,g);if(p){let m=this.cleanTwitterContent(p),u=this.parseTwitterThread(m);t.querySelectorAll(".twitter-card").forEach(v=>v.remove()),u.forEach((v,f)=>{let T=`Thread ${f+1}/${u.length}`,b=this.createTwitterCard(v,T,!0);b.dataset.platform="thread",b.dataset.threadId=e,t.appendChild(b)});let h=t.querySelector(".current-length");h&&(h.textContent=this.getTotalChars(u));let w=t.querySelector(".master-length-slider");w&&(w.value=this.getTotalChars(u)),console.log("\u2705 Thread regenerated successfully")}}catch(s){console.error("Error regenerating thread:",s),alert("Failed to regenerate thread. Please try again.")}finally{o.textContent=r,o.disabled=!1}},copyAllTweets:async function(t,e,n){try{let a=t.join(`

`);await navigator.clipboard.writeText(a);let i=e.innerHTML;e.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          <polyline points="20,6 9,17 4,12"></polyline>
        </svg>`,e.classList.add("success"),this.showToast&&this.showToast("All tweets copied to clipboard!"),setTimeout(()=>{e.innerHTML=i,e.classList.remove("success")},2e3)}catch(a){console.error("Failed to copy all tweets:",a),this.showToast&&this.showToast("Failed to copy tweets")}},saveAllTweets:async function(t,e,n,a){if(!window.FibrStorage){this.showToast&&this.showToast("Gallery storage not available");return}try{let i=e.innerHTML,o=t.map((s,l)=>`${l+1}/${t.length}:
${s}`).join(`

---

`),r={id:n,type:"thread",platform:"thread",title:a,url:this.currentTab?.url||"",domain:this.currentDomain||"",content:o,tweets:t.map((s,l)=>({id:`tweet_${l+1}`,number:`${l+1}/${t.length}`,content:s,charCount:this.getAccurateCharacterCount?this.getAccurateCharacterCount(s):s.length})),rawContent:t.join(`
...
`),totalTweets:t.length,totalChars:this.getTotalChars?this.getTotalChars(t):t.join("").length,isAutoSaved:!1,timestamp:Date.now(),updatedAt:Date.now(),isThread:!0,hasThreadStructure:t.length>1};await window.FibrStorage.saveContent("twitter",r),e.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    `,document.body.appendChild(e),requestAnimationFrame(()=>{e.style.opacity="1",e.querySelector(".rich-text-modal-content").style.transform="scale(1)"}),this._bindEditorEvents(e,t)},_prepareDisplayContent(t){let e=null;if(Array.isArray(t.tweets)&&t.tweets.length>0)e=t.tweets.map(o=>{let s=(o.content||"").toString().replace(/^\d+\/[nN\d]+[\s:]*/,"").trim();return{content:s,charCount:o.charCount||this._getCharCount(s)}});else if(t.content&&window.FibrTwitter&&window.FibrTwitter.parseTwitterThread){let o=window.FibrTwitter.parseTwitterThread(t.content||"");Array.isArray(o)&&o.length>1&&(e=o.map(r=>({content:r,charCount:this._getCharCount(r)})))}let n=Array.isArray(e)&&e.length>0,a=t.totalChars||this._getCharCount(t.content||""),i="";return n?(i='<div style="display: flex; flex-direction: column; gap: 16px;">',e.forEach((o,r)=>{i+=`
          <div style="
            background: var(--secondary-bg);
            border: 1px solid var(--border-color);
            border-radius: 8px;
      ...
      `,{contentHTML:i,meta:`${this._formatDate(t.updatedAt||t.timestamp)} \u2022 ${a} characters${n?` \u2022 ${e.length} tweets`:""}`,tweetsArray:e,isThread:n}},_prepareEditableContent(t){let e=t.content||"";if(Array.isArray(t.tweets)&&t.tweets.length>0)e=t.tweets.map(n=>(n.content||"").toString().replace(/^\d+\/[nN\d]+[\s:]*/,"").trim()).join(`
