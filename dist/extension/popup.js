...
`).trim();if(!s||s.length<20){let o=[/STOP.*[\s\S]*?#[A-Za-z]+/i,/🤯.*[\s\S]*?#[A-Za-z]+/i,/\(1\/\d+\).*[\s\S]*?#[A-Za-z]+/i];for(let r of o){let c=t.match(r);if(c&&c[0].length>30){s=c[0].trim();break}}}return s||"Unable to extract clean post content. Please try generating again."},setLoading:function(e,t="..."){this.isLoading=e,e?(this.pageStatus&&(this.pageStatus.textContent=t),this.setAriaStatus(t)):(this.pageStatus&&!this.pageStatus.textContent.startsWith("\u2705")&&(this.pageStatus.textContent="\u2705 Done"),this.setAriaStatus("Ready"))},updateQuickActionsVisibility:function(){this.quickActions&&this.quickActions.classList.toggle("hidden",!this.pageContent)},resetScreenForGeneration:function(){this.sidebar&&(this.sidebar.classList.add("hidden"),this.sidebar.style.display="none"),this.messagesContainer&&(this.messagesContainer.innerHTML=""),this.updateQuickActionsVisibility()},renderCard:function(e,t,n={}){let a=document.createElement("div");a.className="twitter-content-container";let i=document.createElement("div");i.className="twitter-card analytics-card",i.dataset.contentType=n.contentType||"content",i.dataset.contentId=n.contentId||Date.now().toString();let s={summary:"\u{1F4DD}",keypoints:"\u{1F511}",analysis:"\u{1F4CA}",faq:"\u2753",factcheck:"\u2705",blog:"\u{1F4F0}",proscons:"\u2696\uFE0F",timeline:"\u{1F4C5}",quotes:"\u{1F4AC}"},o=n.contentType||"content",r=s[o]||"\u{1F4C4}",c=n.markdown?`data-markdown="${encodeURIComponent(n.markdown)}"`:"";if(i.innerHTML=`
      ...
      `,window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let m=n.contentType||"content",u={id:n.contentId||Date.now().toString(),content:n.markdown||t,title:e},y=i.querySelector(".twitter-header-actions");y&&window.TabTalkUI.addSaveButtonToCard(i,y,m,u)}let l=i.querySelector(".copy-btn"),g=l.innerHTML;l.addEventListener("click",async m=>{m.stopPropagation();try{let u=i.querySelector(".structured-html"),y=u?.getAttribute("data-markdown"),p=y?decodeURIComponent(y):u?.innerText||"",f=i.dataset.imagePrompt?decodeURIComponent(i.dataset.imagePrompt):null;f&&(p+=`

---
\u{1F5BC}\uFE0F Nano Banana Prompt (9:16):
`+f),await navigator.clipboard.writeText(p),l.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="20,6 9,17 4,12"></polyline>
          </svg>`,l.classList.add("success"),setTimeout(()=>{l.innerHTML=g,l.classList.remove("success")},2e3)}catch(u){console.error("Copy failed:",u)}}),a.appendChild(i);let d=n.container||this.messagesContainer||document.getElementById("messages-container");return d&&(d.appendChild(a),d===this.messagesContainer&&d.scrollTo({top:d.scrollHeight,behavior:"smooth"})),i},showProgressBar:function(e){this.hideProgressBar();let t=document.createElement("div");t.className="progress-container",t.id="global-progress",t.innerHTML=`
...
Share your authentic thread now: Generation ID: ${Date.now()}`;else{this.showToast?this.showToast("\u274C Only Twitter/X Post and Twitter Thread are supported.",3e3):alert("\u274C Only Twitter/X Post and Twitter Thread are supported.");return}let c=await this.callGeminiAPIWithSystemPrompt(i,s);if(c){console.log(`TabTalk AI: Successfully generated ${e} content, response length: ${c.length} characters`);let l=this.cleanTwitterContent(c),g=null;if(n){this.showProgressBar("Generating image prompt...");try{if(window.TabTalkImagePromptGenerator){let d=`content_${Date.now()}`;g=await window.TabTalkImagePromptGenerator.generatePromptForCard(d,l),console.log("Image prompt generated:",g?"Success":"Failed")}}catch(d){console.error("Image prompt generation failed:",d)}}if(this.addTwitterMessage("assistant",l,e,g),this.addToHistory){let d={timestamp:new Date().toISOString(),url:this.currentTab?.url||"",title:this.currentTab?.title||"",domain:this.currentDomain||"",content:l,type:e,imagePrompt:g||void 0};await this.addToHistory(e,d)}await this.saveState()}else throw new Error("Empty response received from Gemini API")}catch(a){console.error("Error generating social content:",a),console.error("Error details:",{message:a.message,stack:a.stack,platform:e,hasApiKey:!!this.apiKey,hasPageContent:!!this.pageContent,pageContentLength:this.pageContent?.length}),this.showToast?this.showToast(`\u274C Error: ${a.message}. Please check your API key and try again.`,4e3):alert(`\u274C Error generating social media content: ${a.message}. Please check your API key and try again.`)}finally{this.setLoading(!1),this.hideProgressBar()}},showProgressBar:function(e){this.hideProgressBar();let t=document.createElement("div");t.className="progress-container",t.id="twitter-progress",t.innerHTML=`
        <div class="progress-message">${e}</div>
      ...
      `,this.messagesContainer.appendChild(t),this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"}),setTimeout(()=>{let n=t.querySelector(".progress-fill");n&&(n.style.width="100%")},100)},hideProgressBar:function(){let e=document.getElementById("twitter-progress");e&&e.remove()},addTwitterMessage:function(e,t,n,a=null){this.renderTwitterContent(t,n,a)},renderTwitterContent:function(e,t,n=null){let a=document.createElement("div");if(a.className="twitter-content-container",t==="thread"){let i=this.parseTwitterThread(e);i.length<=1&&e.includes("1/")&&(console.warn("\u26A0\uFE0F  Thread parsing may have failed - got single tweet but content suggests thread"),console.log("Original content length:",e.length),console.log("Parsed tweets count:",i.length));let s=`thread_${Date.now()}`;this.autoSaveThread(s,i,e);let o=document.createElement("div");o.className="thread-header";let r=this.getTotalChars(i);o.innerHTML=`
        ...
        `,a.appendChild(o);let c=document.createElement("div");c.className="thread-master-control",c.innerHTML=`
...
`+i:a=i;return a.trim()&&t.push(a.trim()),t.filter(i=>i.length>0)},tryFlexiblePatternParsing:function(e){let t=[],n=[/(?:^|\n)(\d+\/\d+)\s*[:\n]\s*([^]*?)(?=\n\d+\/\d+|\n*$)/g,/(?:^|\n)(\d+\/\d+)\s*\n\s*([^]*?)(?=\n\d+\/\d+|\n*$)/g,/(?:^|\n)(\d+)\/(\d+)\s*[:\n]\s*([^]*?)(?=\n\d+\/\d+|\n*$)/g];for(let a of n){let i;for(t.length=0;(i=a.exec(e))!==null;){let s=i[2]||i[1]||"";s.trim()&&t.push(s.trim())}if(t.length>1)break}return t.filter(a=>a.length>0)},tryContentBasedSplitting:function(e){let t=[],n=e.includes("\u{1F9F5}")||e.toLowerCase().includes("thread")||e.length>500,a=e.split(/\n\s*\n|\n---\n/).filter(s=>s.trim());if(a.length>1&&n)for(let s of a){let o=s.trim();o.length>15&&!o.match(/^🧵\s*thread\s*on\s*.*$/i)&&!o.match(/^\d+\.\s*$/)&&t.push(o)}if(t.length<=1&&e.length>600){let s=e.match(/[^.!?]+[.!?]+/g)||[e],o="";for(let r of s)this.getAccurateCharacterCount(o+r)<=280?o+=r:(o.trim()&&t.push(o.trim()),o=r);o.trim()&&t.push(o.trim())}let i=t.filter(s=>{let o=s.trim();return o.length>20&&!o.match(/^🧵\s*thread\s*on\s*.*$/i)&&!o.match(/^\d+\.\s*$/)});return i.length<2&&a.length<=2?[e.trim()]:i.length>0?i:[e.trim()]},createTwitterCard:function(e,t,n=!1,a=null){let i=document.createElement("div");i.className="twitter-card";let s=this.currentSelectedTone?`
      ...
      `,window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let m={id:Date.now().toString(),content:e,title:t},u=t.toLowerCase().includes("thread")?"thread":"twitter",y=i.querySelector(".twitter-header-actions");y&&window.TabTalkUI.addSaveButtonToCard(i,y,u,m)}let c=i.querySelector(".copy-btn"),l=i.querySelector(".twitter-text"),g=c.innerHTML;c.addEventListener("click",async m=>{m.stopPropagation();try{let u=l.value,y=i.dataset.imagePrompt?decodeURIComponent(i.dataset.imagePrompt):null,p=a||y;p&&(u+=`

---
\u{1F5BC}\uFE0F Nano Banana Prompt (9:16):
`+p),await navigator.clipboard.writeText(u),c.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="20,6 9,17 4,12"></polyline>
          </svg>`,c.classList.add("success"),setTimeout(()=>{c.innerHTML=g,c.classList.remove("success")},2e3)}catch(u){console.error("Copy failed:",u)}});let d=()=>{l.style.height="auto",l.style.height=Math.max(80,l.scrollHeight)+"px"};if(setTimeout(d,0),l.addEventListener("input",()=>{let m=i.querySelector(".twitter-char-count"),u=this.getAccurateCharacterCount(l.value);m.textContent=`${u} characters`,m.style.color="var(--text-secondary)",d()}),!n){let m=i.querySelector(".length-slider"),u=i.querySelector(".length-display"),y=i.querySelector(".regenerate-btn");m&&u&&m.addEventListener("input",()=>{u.textContent=m.value}),i.dataset.originalContent=this.pageContent,i.dataset.platform=t.includes("Thread")?"thread":"twitter",this.currentSelectedTone&&(i.dataset.selectedTone=JSON.stringify(this.currentSelectedTone)),y&&y.addEventListener("click",async()=>{let p=parseInt(m.value),f=i.dataset.platform,v=i.dataset.selectedTone?JSON.parse(i.dataset.selectedTone):this.currentSelectedTone;await this.regenerateWithLength(i,p,f,{selectedTone:v})})}return i},cleanTwitterContent:function(e){if(!e)return e;let t=e;t=t.replace(/^.*?Unacceptable.*?\n/gim,""),t=t.replace(/^.*?critical failure.*?\n/gim,""),t=t.replace(/^.*?forbidden.*?formatting.*?\n/gim,""),t=t.replace(/^.*?breaks the instructions.*?\n/gim,""),t=t.replace(/^.*?--[•\-]\s*Original Response:.*?\n/gim,""),t=t.replace(/^.*?You have used.*?\n/gim,""),t=t.replace(/^.*?This output is unusable.*?\n/gim,""),t=t.replace(/^.*?Here's your.*?content:.*?\n/gim,""),t=t.replace(/^.*?OUTPUT:.*?\n/gim,""),t=t.replace(/^.*?here's a rephrased version.*?\n/gim,""),t=t.replace(/^.*?rephrased version.*?\n/gim,""),t=t.replace(/^.*?aiming for.*?tone.*?\n/gim,""),t=t.replace(/^.*?preserving the original.*?\n/gim,""),t=t.replace(/^.*?while preserving.*?\n/gim,""),t=t.replace(/^.*?Okay, here's.*?\n/gim,""),t=t.replace(/^.*?Here's a.*?rephrased.*?\n/gim,""),t=t.replace(/^.*?rephrased.*?version.*?\n/gim,""),t=t.replace(/@[a-zA-Z0-9_]+/g,""),t=t.replace(/^[a-zA-Z0-9_]+:\s*/gm,""),t=t.replace(/\(?@[a-zA-Z0-9_]+\)?/g,""),t=t.replace(/\bby\s+@[a-zA-Z0-9_]+/gi,""),t=t.replace(/\bfrom\s+@[a-zA-Z0-9_]+/gi,""),t=t.replace(/\bvia\s+@[a-zA-Z0-9_]+/gi,""),t=t.replace(/\s+[^.!?]*\?$/gm,""),t=t.replace(/\s+(what do you think\?|what are your thoughts\?|what about you\?|and you\?|right\?|don't you think\?)$/gim,""),t=t.replace(/\n\s*[^.!?]*\?\s*$/gm,""),t=t.replace(/\s+(thoughts\?|opinions\?|ideas\?|comments\?)$/gim,""),t=t.replace(/#\w+/g,""),t=t.replace(/#/g,""),t=t.replace(/\*\*([^*]+)\*\*/g,"$1"),t=t.replace(/\*([^*]+)\*/g,"$1"),t=t.replace(/_{2,}([^_]+)_{2,}/g,"$1"),t=t.replace(/_([^_]+)_/g,"$1"),t=t.replace(/\*{2,}/g,""),t=t.replace(/_{2,}/g,""),t=t.replace(/\(line break\)/gi,`
...
${d.researchContext}`,c=`Share your genuine thoughts about this content in ${t} characters - like you're talking to friends.
...
\u2713 Apply the ${l.name} tone authentically
...
RESEARCH AUGMENTATION:
${d.researchContext}`,c=`Share your thoughts about this content as a Twitter thread - like you're telling a story to friends.
...
Generate your expert research thread now:`,l=await this.callGeminiAPIWithSystemPrompt(r,c);if(l){let g=this.cleanTwitterContent(l),d=this.parseTwitterThread(g);e.querySelectorAll(".twitter-card").forEach(f=>f.remove()),d.forEach((f,v)=>{let w=`Thread ${v+1}/${d.length}`,T=this.createTwitterCard(f,w,!0);T.dataset.platform="thread",T.dataset.threadId=t,e.appendChild(T)});let u=e.querySelector(".thread-meta");u&&(u.textContent=`${d.length} tweets \u2022 ${this.getTotalChars(d)} chars`);let y=e.querySelector(".current-length");y&&(y.textContent=this.getTotalChars(d));let p=e.querySelector(".master-length-slider");p&&(p.value=this.getTotalChars(d)),await this.autoSaveThread(t,d,g),console.log("\u2705 Thread regenerated successfully")}}catch(o){console.error("Error regenerating thread:",o),alert("Failed to regenerate thread. Please try again.")}finally{i.textContent=s,i.disabled=!1}},escapeHtml:function(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}};window.TabTalkTwitter=h})();(function(){let h={knowledgePacks:{},modalInitialized:!1,popupInstance:null,init:function(){this.modalInitialized||(this.createModalHTML(),this.bindModalEvents(),this.modalInitialized=!0)},createModalHTML:function(){document.getElementById("thread-generator-modal")||document.body.insertAdjacentHTML("beforeend",`
      ...
      `)},bindModalEvents:function(){let e=document.getElementById("thread-generator-modal");if(!e)return;let t=e.querySelector(".tone-modal-close"),n=e.querySelector(".tone-modal-overlay"),a=document.getElementById("thread-gen-cancel-btn"),i=document.getElementById("thread-gen-generate-btn");t?.addEventListener("click",()=>this.hideModal()),n?.addEventListener("click",()=>this.hideModal()),a?.addEventListener("click",()=>this.hideModal()),i?.addEventListener("click",()=>this.handleGenerate()),e.addEventListener("keydown",s=>{s.key==="Escape"&&this.hideModal()})},showModal:function(e){if(e)h.popupInstance=e,console.log("ThreadGenerator: Stored popup instance, has apiKey:",!!e.apiKey);else{console.error("ThreadGenerator: No popup instance provided to showModal"),alert("Unable to open thread generator. Please refresh and try again.");return}h.init();let t=document.getElementById("thread-generator-modal");if(!t)return;t.classList.remove("hidden"),t.setAttribute("aria-hidden","false"),document.getElementById("modal-thread-topic")?.focus()},hideModal:function(){let e=document.getElementById("thread-generator-modal");e&&(e.classList.add("hidden"),e.setAttribute("aria-hidden","true"))},handleGenerate:async function(){let e=document.getElementById("modal-thread-topic")?.value?.trim(),t=document.getElementById("modal-use-knowledge-pack")?.checked;if(!e){alert("Please enter a topic");return}console.log("ThreadGenerator: handleGenerate called"),console.log("ThreadGenerator: popupInstance exists:",!!h.popupInstance),console.log("ThreadGenerator: popupInstance has apiKey:",!!h.popupInstance?.apiKey),console.log("ThreadGenerator: popupInstance has generateThreadMVP:",!!h.popupInstance?.generateThreadMVP),h.hideModal(),h.popupInstance&&h.popupInstance.resetScreenForGeneration&&h.popupInstance.resetScreenForGeneration(),h.popupInstance&&h.popupInstance.generateThreadMVP?await h.popupInstance.generateThreadMVP(e,{useKnowledgePack:t,maxTweets:8,tone:"curious"}):(console.error("Popup instance not available for thread generation"),console.error("popupInstance:",h.popupInstance),alert("Unable to generate thread. Please try again."))},loadKnowledgePack:async function(e){if(this.knowledgePacks[e])return this.knowledgePacks[e];try{let t=await fetch(`knowledge-packs/${e}.json`);if(!t.ok)return console.warn(`Knowledge pack not found for ${e}`),null;let n=await t.json();return this.knowledgePacks[e]=n,n}catch(t){return console.error(`Error loading knowledge pack for ${e}:`,t),null}},getRandomHook:function(e){if(!e||!e.hooks||e.hooks.length===0)return null;let t=Math.floor(Math.random()*e.hooks.length);return e.hooks[t]},optimizeThreadLength:async function(e){try{let t=`Analyze this topic and determine the optimal Twitter thread length: "${e}"
...
`),n.showProgressBar&&n.showProgressBar("Generating thread...");let r="You are a precise thread outline creator. You create structured outlines for engaging Twitter/X threads. No markdown, no hashtags.",c=`Create a ${i}-tweet thread outline about: ${e}
...
Generate the outline now:`,l=await n.callGeminiAPIWithSystemPrompt(r,c);if(!l)throw new Error("Failed to generate outline");console.log("\u2705 Outline generated");let g="You are a masterful Twitter/X thread storyteller. You craft threads that captivate readers. Each tweet pulses with energy and personality. Write in plain text with strategic emojis - no hashtags, no URLs, no formatting symbols.",d=`Transform this outline into a complete ${i}-tweet thread about: ${e}
...
Generate the complete thread now:`,m=await n.callGeminiAPIWithSystemPrompt(g,d);if(!m)throw new Error("Failed to expand thread");console.log("\u2705 Thread expanded");let u=n.cleanTwitterContent(m),y=n.parseTwitterThread(u),p=[];for(let v of y)if(n.getAccurateCharacterCount(v)<=280)p.push(v);else{let T=await h.smartSplitTweet.call(n,v,280);p.push(...T)}console.log(`\u2705 Thread generated: ${p.length} tweets`);let f=`thread_${Date.now()}`;h.renderThreadGeneratorResult.call(n,p,f,e,a),n.autoSaveThread&&await n.autoSaveThread(f,p,u),await n.saveState()}catch(o){console.error("Error generating thread:",o),alert(`\u274C Error generating thread: ${o.message}`)}finally{n.setLoading(!1),n.hideProgressBar&&n.hideProgressBar()}},smartSplitTweet:async function(e,t){let n=e.match(/[^.!?]+[.!?]+/g)||[e],a=[],i="";for(let s of n)this.getAccurateCharacterCount(i+s)<=t?i+=s:(i&&a.push(i.trim()),i=s);return i&&a.push(i.trim()),a.length>0?a:[e.substring(0,t)]},renderThreadGeneratorResult:function(e,t,n,a=!0){let i=document.createElement("div");i.className="twitter-content-container thread-generator-result",i.dataset.topic=n,i.dataset.useKnowledgePack=a;let s=document.createElement("div");s.className="thread-header";let o=this.getTotalChars(e);s.innerHTML=`
        <div class="thread-info">
          <span class="thread-icon">\u{1F9F5}</span>
      ...
      `,i.appendChild(s);let r=s.querySelector(".btn-copy-all-thread"),c=s.querySelector(".copy-all-status");r.addEventListener("click",async()=>{await this.copyAllTweets(e,r,c,t)});let l=document.createElement("div");l.className="thread-master-control",l.innerHTML=`
        <div class="master-control-header">
          <span class="control-label">Thread Length Control</span>
          <span class="control-hint">Adjust total thread length \u2022 Characters distributed proportionally</span>
        </div>
      ...
      `,i.appendChild(l);let g=l.querySelector(".master-length-slider"),d=l.querySelector(".current-length"),m=l.querySelector(".btn-regenerate-thread"),u=l.querySelectorAll(".preset-btn");g.addEventListener("input",p=>{d.textContent=p.target.value}),u.forEach(p=>{p.addEventListener("click",()=>{let f=p.dataset.length;g.value=f,d.textContent=f})}),m.addEventListener("click",async()=>{let p=parseInt(g.value);await this.regenerateEntireThreadForGenerator(i,t,p,n,a)});let y=document.getElementById("modal-include-image-prompts")?.checked;e.forEach((p,f)=>{let v=`Thread ${f+1}/${e.length}`,w=this.createTwitterCard(p,v,!0);w.dataset.platform="thread",w.dataset.threadId=t,i.appendChild(w),y&&window.TabTalkImagePromptGenerator&&(async()=>{try{let T=`threadgen_${t}_tweet_${f+1}`,b=await window.TabTalkImagePromptGenerator.generatePromptForCard(T,p);if(b){w.dataset.imagePrompt=encodeURIComponent(b);let k=w.querySelector(".twitter-card-content");if(k&&!w.querySelector(".image-prompt-display")){let C=document.createElement("div");C.className="image-prompt-display",C.innerHTML=`
                    <div class="image-prompt-label">\u{1F5BC}\uFE0F Nano Banana Prompt (9:16)</div>
                    <div class="image-prompt-text">${window.TabTalkUI?.escapeHtml?window.TabTalkUI.escapeHtml(b):b}</div>
                  `,k.appendChild(C)}}}catch(T){console.warn("Thread Generator: image prompt generation failed:",T)}})()}),this.messagesContainer.appendChild(i),setTimeout(()=>{this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"})},100)},regenerateEntireThreadForGenerator:async function(e,t,n,a,i){let s=e.querySelector(".btn-regenerate-thread");if(!s)return;let o=s.textContent;s.textContent="\u23F3 Regenerating...",s.disabled=!0;try{let r=Math.max(3,Math.min(12,Math.ceil(n/400))),c="";i&&(c=`
...
Write in plain text with precise, professional language - no hashtags, no URLs, no formatting symbols. Pure expert-level analysis with strategic emojis that emphasize key insights.`,l=`Generate a comprehensive, expert-level research thread on: ${n}
...
Generate your expert research thread now:`,g=await this.callGeminiAPIWithSystemPrompt(c,l);if(g){let d=this.cleanTwitterContent(g),m=this.parseTwitterThread(d);this.displayThreadResult(d,n,category),this.saveThreadToGallery(d,n,category),window.TabTalkUI?.showToast("Expert thread generated successfully!",2e3)}}catch(i){console.error("Thread generation failed:",i),window.TabTalkUI?.showToast("Failed to generate thread",3e3)}finally{let i=document.getElementById("generate-thread-btn");i&&(i.textContent="\u{1F680} Generate Enhanced Thread",i.disabled=!1)}}};window.TabTalkThreadGenerator=h})();(function(){let h={initializeHorizontalScroll:function(){let e=document.querySelector(".scroll-container"),t=document.getElementById("scroll-left"),n=document.getElementById("scroll-right");if(!e||!t||!n)return;let a=200;t.addEventListener("click",()=>{e.scrollBy({left:-a,behavior:"smooth"})}),n.addEventListener("click",()=>{e.scrollBy({left:a,behavior:"smooth"})});let i=()=>{let c=e.scrollWidth-e.clientWidth;t.disabled=e.scrollLeft<=0,n.disabled=e.scrollLeft>=c};e.addEventListener("scroll",i),i(),e.addEventListener("wheel",c=>{c.deltaY!==0&&(c.preventDefault(),e.scrollLeft+=c.deltaY,i())});let s=!1,o,r;e.addEventListener("mousedown",c=>{s=!0,o=c.pageX-e.offsetLeft,r=e.scrollLeft,e.style.cursor="grabbing"}),e.addEventListener("mouseleave",()=>{s=!1,e.style.cursor="grab"}),e.addEventListener("mouseup",()=>{s=!1,e.style.cursor="grab",i()}),e.addEventListener("mousemove",c=>{if(!s)return;c.preventDefault();let g=(c.pageX-e.offsetLeft-o)*1.5;e.scrollLeft=r-g}),e.style.cursor="grab"}};window.TabTalkScroll=h})();(function(){let h={INIT_KEY:"savedContent",async loadSaved(e="twitter"){if(!window.TabTalkStorage||!TabTalkStorage.getSavedContent)return console.error("Gallery: TabTalkStorage not available"),[];let t=await TabTalkStorage.getSavedContent();return t?e==="all"?Object.entries(t).flatMap(([a,i])=>Array.isArray(i)?i.map(s=>({...s,_category:a})):[]):Array.isArray(t[e])?t[e]:[]:[]},async render(e,t="twitter"){e.innerHTML="";let n=document.createElement("div");n.className="gallery-header",n.innerHTML=`
      ...
      `;let o=t.querySelector(".gallery-preview"),r=t.querySelector(".btn-action.copy"),c=t.querySelector(".btn-action.read"),l=t.querySelector(".btn-action.edit"),g=t.querySelector(".btn-action.delete");return r.addEventListener("click",async d=>{d.stopPropagation();try{let m="";(window.TabTalkTwitter&&window.TabTalkTwitter.isThreadContent?window.TabTalkTwitter.isThreadContent(e):this.fallbackThreadDetection(e))?m=this.extractThreadContent(e):m=e.content||"",await navigator.clipboard.writeText(m);let y=r.querySelector("span");y.textContent="\u2713",r.classList.add("success"),setTimeout(()=>{y.textContent="Copy",r.classList.remove("success")},1500)}catch(m){console.error("Gallery copy failed",m)}}),c.addEventListener("click",d=>{d.stopPropagation(),this.openReadModal(e)}),l.addEventListener("click",d=>{d.stopPropagation(),this.openEditModal(e)}),g.addEventListener("click",async d=>{d.stopPropagation(),confirm("Delete this saved item?")&&(await this.deleteItem(e),t.remove())}),t.addEventListener("click",d=>{d.target.closest(".btn-action")||this.openReadModal(e)}),t},openReadModal(e){let t=document.createElement("div");t.className="gallery-modal";let n="";t.innerHTML=`
...
- Do NOT add your own analysis or commentary`,keywords:["rephrase","enhance","improve","professional","polished"]},"hypocrite-buster":{id:"hypocrite-buster",name:"Hypocrite Buster",icon:"\u{1F3AF}",color:"var(--accent-light)",category:"reply",subcategory:"critical",description:"Point out contradictions and double standards",example:"Interesting how they ignore their own past stance...",aiInstructions:`TONE: Hypocrite Buster
- Identify contradictions or double standards in the content
- Point out when arguments conflict with obvious counterexamples
...
- Use irony and juxtaposition to highlight contradictions effectively`,keywords:["contradiction","double-standards","inconsistency","critical","exposure"]}},customTones:[],sessionCache:{lastSelectedTone:null,customCombinations:[]},init:function(){this.loadCustomTones(),this.createModalHTML(),this.bindModalEvents()},loadCustomTones:async function(){try{let e=await chrome.storage.local.get("customTones");e.customTones&&(this.customTones=e.customTones)}catch(e){console.error("Error loading custom tones:",e)}},saveCustomTones:async function(){try{await chrome.storage.local.set({customTones:this.customTones})}catch(e){console.error("Error saving custom tones:",e)}},createModalHTML:function(){let e=`
      ...
      `;document.getElementById("tone-selector-modal")||document.body.insertAdjacentHTML("beforeend",e)},renderToneGrid:function(){let e=Object.values(this.toneDefinitions).filter(a=>a.category==="original"),t=Object.values(this.toneDefinitions).filter(a=>a.category==="reply"),n=(a,i,s)=>`
        <div class="tone-category">
          <div class="category-header">
            <span class="category-icon">${s}</span>
            <span class="category-title">${i}</span>
          </div>
          <div class="tone-grid-row">
            ${a.map(o=>`
              <div class="tone-option" 
                   ...
                   data-subcategory="${o.subcategory}"
                   role="radio"
                   aria-checked="false"
                   tabindex="0">
                <div class="tone-icon">${o.icon}</div>
                <div class="tone-info">
                  <div class="tone-name">${o.name}</div>
                  <div class="tone-description">${o.description}</div>
                </div>
      ...
      `},renderToneOptions:function(){return Object.values(this.toneDefinitions).map(e=>`<option value="${e.id}">${e.icon} ${e.name}</option>`).join("")},bindModalEvents:function(){let e=document.getElementById("tone-selector-modal");if(!e)return;e.querySelector(".tone-modal-close")?.addEventListener("click",()=>this.hideModal()),e.querySelector(".tone-modal-overlay")?.addEventListener("click",()=>this.hideModal()),document.getElementById("tone-cancel-btn")?.addEventListener("click",()=>this.hideModal()),e.querySelectorAll(".tone-option").forEach(d=>{d.addEventListener("click",()=>this.selectTone(d)),d.addEventListener("keydown",m=>{(m.key==="Enter"||m.key===" ")&&(m.preventDefault(),this.selectTone(d))})}),document.getElementById("tone-generate-btn")?.addEventListener("click",()=>this.handleGenerate()),document.getElementById("toggle-custom-builder")?.addEventListener("click",()=>this.toggleCustomBuilder());let r=document.getElementById("custom-tone-1"),c=document.getElementById("custom-tone-2");r?.addEventListener("change",()=>this.updateCustomPreview()),c?.addEventListener("change",()=>this.updateCustomPreview()),document.getElementById("save-custom-tone")?.addEventListener("click",()=>this.saveCustomCombination()),document.getElementById("use-custom-tone")?.addEventListener("click",()=>this.useCustomCombination()),e.addEventListener("keydown",d=>{d.key==="Escape"&&this.hideModal()})},showModal:async function(e,t){let n=document.getElementById("tone-selector-modal");if(!n)return;this.currentPlatform=e,this.currentPageContent=t,n.classList.remove("hidden"),n.setAttribute("aria-hidden","false"),n.querySelector(".tone-option")?.focus(),await this.generateRecommendations(t),this.renderSavedCustomTones()},hideModal:function(){let e=document.getElementById("tone-selector-modal");e&&(e.classList.add("hidden"),e.setAttribute("aria-hidden","true"),this.resetSelections())},selectTone:function(e){document.querySelectorAll(".tone-option").forEach(a=>{a.classList.remove("selected"),a.setAttribute("aria-checked","false")}),e.classList.add("selected"),e.setAttribute("aria-checked","true"),this.selectedToneId=e.dataset.toneId,this.selectedTone=this.toneDefinitions[this.selectedToneId];let n=document.getElementById("tone-generate-btn");n&&(n.disabled=!1),this.sessionCache.lastSelectedTone=this.selectedToneId},generateRecommendations:async function(e){let t=document.getElementById("tone-recommendations"),n=document.getElementById("recommended-tones");if(!(!t||!n))try{t.classList.remove("hidden"),n.innerHTML='<div class="recommendations-loading">Analyzing content...</div>';let a=await this.analyzeContentForTones(e);a.length>0?(n.innerHTML=a.map(s=>`
          ...
          `).join(""),n.querySelectorAll(".recommended-tone").forEach(s=>{s.addEventListener("click",()=>{let o=s.dataset.toneId,r=document.querySelector(`.tone-option[data-tone-id="${o}"]`);r&&(this.selectTone(r),r.scrollIntoView({behavior:"smooth",block:"center"}))})})):n.innerHTML='<div class="no-recommendations">All tones work well for this content!</div>'}catch(a){console.error("Error generating recommendations:",a),n.innerHTML='<div class="recommendations-error">Could not analyze content</div>'}},analyzeContentForTones:async function(e){let t=e.toLowerCase(),n=[],a=/controversy|debate|disagree|conflict|dispute/i.test(e),i=/data|statistics|study|research|evidence|percent|number/i.test(e),s=/claim|assert|state|argue|maintain/i.test(e),o=/success|achievement|breakthrough|innovation|progress/i.test(e),r=/problem|issue|concern|risk|danger|failure/i.test(e),c=/funny|joke|ironic|amusing|hilarious/i.test(e),l=/future|upcoming|next|will|plan|forecast/i.test(e),g=/warning|caution|beware|careful|risk/i.test(e),d=e.length,m=e.split(/\s+/).length;return a&&i&&n.push({toneId:"contradictory",...this.toneDefinitions.contradictory,reason:"Content contains controversial claims with data - perfect for evidence-based contradiction",confidence:92}),s&&!i&&n.push({toneId:"fact-check",...this.toneDefinitions["fact-check"],reason:"Multiple claims detected without strong evidence - ideal for fact-checking",confidence:88}),o&&i&&n.push({toneId:"agreeing",...this.toneDefinitions.agreeing,reason:"Positive developments backed by data - great for supportive agreement",confidence:90}),a&&c&&n.push({toneId:"trolling",...this.toneDefinitions.trolling,reason:"Controversial topic with humorous elements - perfect for playful fact-based trolling",confidence:85}),r&&!a&&n.push({toneId:"funny",...this.toneDefinitions.funny,reason:"Issues present without heated debate - ideal for humorous take",confidence:83}),i&&d>2e3&&n.push({toneId:"deeper-insights",...this.toneDefinitions["deeper-insights"],reason:"Substantial content with data - perfect for deeper insights analysis",confidence:86}),n.sort((u,y)=>y.confidence-u.confidence).slice(0,3)},toggleCustomBuilder:function(){let e=document.getElementById("custom-tone-builder"),t=document.getElementById("toggle-custom-builder"),n=t?.querySelector(".toggle-arrow");if(e&&t){let a=e.classList.contains("hidden");e.classList.toggle("hidden"),n&&(n.textContent=a?"\u25B2":"\u25BC")}},updateCustomPreview:function(){let e=document.getElementById("custom-tone-1"),t=document.getElementById("custom-tone-2"),n=document.getElementById("custom-tone-preview"),a=document.querySelector(".builder-preview"),i=document.getElementById("save-custom-tone"),s=document.getElementById("use-custom-tone");if(!e||!t||!n)return;let o=e.value,r=t.value;if(o&&r&&o!==r){let c=this.toneDefinitions[o],l=this.toneDefinitions[r];n.innerHTML=`
          ...
          <div class="preview-description">
            ${this.generateCombinedDescription(c,l)}
          </div>
