...
`)),n.trim()},setLoading:function(e,t="..."){this.isLoading=e,e?(this.pageStatus&&(this.pageStatus.textContent=t),this.setAriaStatus(t)):(this.pageStatus&&!this.pageStatus.textContent.startsWith("\u2705")&&(this.pageStatus.textContent="\u2705 Done"),this.setAriaStatus("Ready"))},updateQuickActionsVisibility:function(){this.quickActions&&this.quickActions.classList.toggle("hidden",!this.pageContent)},resetScreenForGeneration:function(){this.sidebar&&(this.sidebar.classList.add("hidden"),this.sidebar.style.display="none"),this.messagesContainer&&(this.messagesContainer.innerHTML=""),this.updateQuickActionsVisibility()},renderCard:function(e,t,n={}){let a=document.createElement("div");a.className="twitter-content-container";let i=document.createElement("div");i.className="twitter-card analytics-card",i.dataset.contentType=n.contentType||"content",i.dataset.contentId=n.contentId||Date.now().toString();let s={summary:"\u{1F4DD}",keypoints:"\u{1F511}",analysis:"\u{1F4CA}",faq:"\u2753",factcheck:"\u2705",blog:"\u{1F4F0}",proscons:"\u2696\uFE0F",timeline:"\u{1F4C5}",quotes:"\u{1F4AC}"},o=n.contentType||"content",r=s[o]||"\u{1F4C4}",c=n.markdown?`data-markdown="${encodeURIComponent(n.markdown)}"`:"";if(i.innerHTML=`
      ...
      `,window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let p=n.contentType||"content",u={id:n.contentId||Date.now().toString(),content:n.markdown||t,title:e},y=i.querySelector(".twitter-header-actions");y&&window.TabTalkUI.addSaveButtonToCard(y,p,u)}let m=i.querySelector(".copy-btn"),l=m.innerHTML;m.addEventListener("click",async p=>{p.stopPropagation();try{let u=i.querySelector(".structured-html"),y=u?.getAttribute("data-markdown"),g=y?decodeURIComponent(y):u?.innerText||"";await navigator.clipboard.writeText(g),m.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="20,6 9,17 4,12"></polyline>
          </svg>`,m.classList.add("success"),setTimeout(()=>{m.innerHTML=l,m.classList.remove("success")},2e3)}catch(u){console.error("Copy failed:",u)}}),a.appendChild(i);let d=n.container||this.messagesContainer;return d.appendChild(a),d===this.messagesContainer&&d.scrollTo({top:d.scrollHeight,behavior:"smooth"}),i},showProgressBar:function(e){this.hideProgressBar();let t=document.createElement("div");t.className="progress-container",t.id="global-progress",t.innerHTML=`
...
Generate your thread now:`;else{this.showToast?this.showToast("\u274C Only Twitter/X Post and Twitter Thread are supported.",3e3):alert("\u274C Only Twitter/X Post and Twitter Thread are supported.");return}let r=await this.callGeminiAPIWithSystemPrompt(a,i);if(r){console.log(`TabTalk AI: Successfully generated ${e} content, response length: ${r.length} characters`);let c=this.cleanTwitterContent(r);if(this.addTwitterMessage("assistant",c,e),this.addToHistory){let m={timestamp:new Date().toISOString(),url:this.currentTab?.url||"",title:this.currentTab?.title||"",domain:this.currentDomain||"",content:c,type:e};await this.addToHistory(e,m)}await this.saveState()}else throw new Error("Empty response received from Gemini API")}catch(n){console.error("Error generating social content:",n),console.error("Error details:",{message:n.message,stack:n.stack,platform:e,hasApiKey:!!this.apiKey,hasPageContent:!!this.pageContent,pageContentLength:this.pageContent?.length}),this.showToast?this.showToast(`\u274C Error: ${n.message}. Please check your API key and try again.`,4e3):alert(`\u274C Error generating social media content: ${n.message}. Please check your API key and try again.`)}finally{this.setLoading(!1),this.hideProgressBar()}},showProgressBar:function(e){this.hideProgressBar();let t=document.createElement("div");t.className="progress-container",t.id="twitter-progress",t.innerHTML=`
      ...
      `,this.messagesContainer.appendChild(t),this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"}),setTimeout(()=>{let n=t.querySelector(".progress-fill");n&&(n.style.width="100%")},100)},hideProgressBar:function(){let e=document.getElementById("twitter-progress");e&&e.remove()},addTwitterMessage:function(e,t,n){this.renderTwitterContent(t,n)},renderTwitterContent:function(e,t){let n=document.createElement("div");if(n.className="twitter-content-container",t==="thread"){let a=this.parseTwitterThread(e),i=`thread_${Date.now()}`;this.autoSaveThread(i,a,e);let s=document.createElement("div");s.className="thread-header";let o=this.getTotalChars(a);s.innerHTML=`
        ...
        `,n.appendChild(r);let c=s.querySelector(".btn-copy-all-thread"),m=s.querySelector(".copy-all-status");c.addEventListener("click",async()=>{await this.copyAllTweets(a,c,m)});let l=r.querySelector(".master-length-slider"),d=r.querySelector(".current-length"),p=r.querySelector(".btn-regenerate-thread"),u=r.querySelectorAll(".preset-btn");l.addEventListener("input",y=>{d.textContent=y.target.value}),u.forEach(y=>{y.addEventListener("click",()=>{let g=y.dataset.length;l.value=g,d.textContent=g})}),p.addEventListener("click",async()=>{let y=parseInt(l.value);await this.regenerateEntireThread(n,i,y,e)}),a.forEach((y,g)=>{let v=`Thread ${g+1}/${a.length}`,w=this.createTwitterCard(y,v,!0);w.dataset.platform=t,w.dataset.threadId=i,n.appendChild(w)})}else{let a=this.createTwitterCard(e,"Post");a.dataset.platform=t,n.appendChild(a)}this.messagesContainer.appendChild(n),setTimeout(()=>{this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"})},100)},parseTwitterThread:function(e){let n=this.cleanTwitterContent(e).replace(/Here's your clean.*?content:\s*/gi,"").trim(),a=[],i=n.split(`
`),s="",o=null;for(let r of i){let c=r.trim(),m=c.match(/^(\d+)\/(\d+)[\s:]*(.*)$/);m?(s.trim()&&a.push(s.trim()),o=m[1],s=m[3]||""):o!==null&&c&&(s+=(s?`
`:"")+c)}return s.trim()&&a.push(s.trim()),a.length===0?(console.warn("Thread parsing failed, returning full content as single tweet"),[n||e]):(console.log(`\u2705 Parsed ${a.length} tweets from thread`),a)},createTwitterCard:function(e,t,n=!1){let a=document.createElement("div");a.className="twitter-card";let i=this.currentSelectedTone?`
      ...
      `,window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let l={id:Date.now().toString(),content:e,title:t},d=t.toLowerCase().includes("thread")?"thread":"twitter",p=a.querySelector(".twitter-header-actions");p&&window.TabTalkUI.addSaveButtonToCard(p,d,l)}let o=a.querySelector(".copy-btn"),r=a.querySelector(".twitter-text"),c=o.innerHTML;o.addEventListener("click",async l=>{l.stopPropagation();try{await navigator.clipboard.writeText(r.value),o.innerHTML=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <polyline points="20,6 9,17 4,12"></polyline>
          </svg>`,o.classList.add("success"),setTimeout(()=>{o.innerHTML=c,o.classList.remove("success")},2e3)}catch(d){console.error("Copy failed:",d)}});let m=()=>{r.style.height="auto",r.style.height=Math.max(80,r.scrollHeight)+"px"};if(setTimeout(m,0),r.addEventListener("input",()=>{let l=a.querySelector(".twitter-char-count"),d=this.getAccurateCharacterCount(r.value);l.textContent=`${d} characters`,l.style.color="var(--text-secondary)",m()}),!n){let l=a.querySelector(".length-slider"),d=a.querySelector(".length-display"),p=a.querySelector(".regenerate-btn");l&&d&&l.addEventListener("input",()=>{d.textContent=l.value}),a.dataset.originalContent=this.pageContent,a.dataset.platform=t.includes("Thread")?"thread":"twitter",this.currentSelectedTone&&(a.dataset.selectedTone=JSON.stringify(this.currentSelectedTone)),p&&p.addEventListener("click",async()=>{let u=parseInt(l.value),y=a.dataset.platform,g=a.dataset.selectedTone?JSON.parse(a.dataset.selectedTone):this.currentSelectedTone;await this.regenerateWithLength(a,u,y,{selectedTone:g})})}return a},cleanTwitterContent:function(e){if(!e)return e;let t=e;return t=t.replace(/^.*?Unacceptable.*?\n/gim,""),t=t.replace(/^.*?critical failure.*?\n/gim,""),t=t.replace(/^.*?forbidden.*?formatting.*?\n/gim,""),t=t.replace(/^.*?breaks the instructions.*?\n/gim,""),t=t.replace(/^.*?--[•\-]\s*Original Response:.*?\n/gim,""),t=t.replace(/^.*?You have used.*?\n/gim,""),t=t.replace(/^.*?This output is unusable.*?\n/gim,""),t=t.replace(/^.*?Here's your.*?content:.*?\n/gim,""),t=t.replace(/^.*?OUTPUT:.*?\n/gim,""),t=t.replace(/#\w+/g,""),t=t.replace(/#/g,""),t=t.replace(/\*\*([^*]+)\*\*/g,"$1"),t=t.replace(/\*([^*]+)\*/g,"$1"),t=t.replace(/_{2,}([^_]+)_{2,}/g,"$1"),t=t.replace(/_([^_]+)_/g,"$1"),t=t.replace(/\*{2,}/g,""),t=t.replace(/_{2,}/g,""),t=t.replace(/\(line break\)/gi,`
...
${d.researchContext}`,c=`Recreate this as an expert ${t}-character Twitter post that combines insight with engagement.
...
\u2713 Apply the ${m.name} tone throughout
...
Transform it now:`;else if(n==="thread"){let u=Math.ceil(t/400);r=`You are an expert Twitter/X thread strategist crafting ${u} tweets (${t} total characters) that combine deep analysis with compelling narrative. Each tweet builds on expert insights while maintaining human warmth and accessibility. Write in plain text with strategic emojis - no hashtags, no URLs, no formatting.
...
RESEARCH AUGMENTATION:
${d.researchContext}`,c=`Recreate this as an expert ${u}-tweet thread (around ${t} characters total).
...
Craft your thread now:`}let p=await this.callGeminiAPIWithSystemPrompt(r,c);if(p){let u=this.cleanTwitterContent(p);if(n==="thread"){let w=this.parseTwitterThread(u)[0]||u;i.value=w}else i.value=u;let y=e.querySelector(".twitter-char-count"),g=this.getAccurateCharacterCount(i.value);y.textContent=`${g} characters`,setTimeout(()=>{i.style.height="auto",i.style.height=Math.max(80,i.scrollHeight)+"px"},0)}}catch(r){console.error("Error regenerating content:",r),alert("Error regenerating content. Please try again.")}finally{s.textContent="\u{1F504}",s.disabled=!1}},getDefaultToneInstructions:function(e){let t={supportive:`TONE: Supportive with Facts
...
Craft your ${n}-character thread now:`,m=await this.callGeminiAPIWithSystemPrompt(r,c);if(m){let l=this.cleanTwitterContent(m),d=this.parseTwitterThread(l);e.querySelectorAll(".twitter-card").forEach(v=>v.remove()),d.forEach((v,w)=>{let f=`Thread ${w+1}/${d.length}`,T=this.createTwitterCard(v,f,!0);T.dataset.platform="thread",T.dataset.threadId=t,e.appendChild(T)});let u=e.querySelector(".thread-meta");u&&(u.textContent=`${d.length} tweets \u2022 ${this.getTotalChars(d)} chars`);let y=e.querySelector(".current-length");y&&(y.textContent=this.getTotalChars(d));let g=e.querySelector(".master-length-slider");g&&(g.value=this.getTotalChars(d)),await this.autoSaveThread(t,d,l),console.log("\u2705 Thread regenerated successfully")}}catch(o){console.error("Error regenerating thread:",o),alert("Failed to regenerate thread. Please try again.")}finally{i.textContent=s,i.disabled=!1}}};window.TabTalkTwitter=h})();(function(){let h={knowledgePacks:{},modalInitialized:!1,popupInstance:null,init:function(){this.modalInitialized||(this.createModalHTML(),this.bindModalEvents(),this.modalInitialized=!0)},createModalHTML:function(){document.getElementById("thread-generator-modal")||document.body.insertAdjacentHTML("beforeend",`
...
Generate the outline now:`,l=await a.callGeminiAPIWithSystemPrompt(c,m);if(!l)throw new Error("Failed to generate outline");console.log("\u2705 Outline generated");let d=`You are a masterful Twitter/X thread storyteller. You craft threads about ${e} that captivate readers. Each tweet pulses with energy and personality. Write in plain text with strategic emojis - no hashtags, no URLs, no formatting symbols.`,p=`Transform this outline into a complete ${s}-tweet thread about: ${t}
...
Generate the complete thread now:`,u=await a.callGeminiAPIWithSystemPrompt(d,p);if(!u)throw new Error("Failed to expand thread");console.log("\u2705 Thread expanded");let y=a.cleanTwitterContent(u),g=a.parseTwitterThread(y),v=[];for(let f of g)if(a.getAccurateCharacterCount(f)<=280)v.push(f);else{let C=await h.smartSplitTweet.call(a,f,280);v.push(...C)}console.log(`\u2705 Thread generated: ${v.length} tweets`);let w=`thread_${Date.now()}`;h.renderThreadGeneratorResult.call(a,v,w,e,t,i),a.autoSaveThread&&await a.autoSaveThread(w,v,y),await a.saveState()}catch(r){console.error("Error generating thread:",r),alert(`\u274C Error generating thread: ${r.message}`)}finally{a.setLoading(!1),a.hideProgressBar&&a.hideProgressBar()}},smartSplitTweet:async function(e,t){let n=e.match(/[^.!?]+[.!?]+/g)||[e],a=[],i="";for(let s of n)this.getAccurateCharacterCount(i+s)<=t?i+=s:(i&&a.push(i.trim()),i=s);return i&&a.push(i.trim()),a.length>0?a:[e.substring(0,t)]},renderThreadGeneratorResult:function(e,t,n,a,i=!0){let s=document.createElement("div");s.className="twitter-content-container thread-generator-result",s.dataset.category=n,s.dataset.topic=a,s.dataset.useKnowledgePack=i;let o=document.createElement("div");o.className="thread-header";let r=this.getTotalChars(e);o.innerHTML=`
        <div class="thread-info">
          <span class="thread-icon">\u{1F9F5}</span>
      ...
      `,s.appendChild(o);let c=o.querySelector(".btn-copy-all-thread"),m=o.querySelector(".copy-all-status");c.addEventListener("click",async()=>{await this.copyAllTweets(e,c,m)});let l=document.createElement("div");l.className="thread-master-control",l.innerHTML=`
        <div class="master-control-header">
          <span class="control-label">Thread Length Control</span>
          <span class="control-hint">Adjust total thread length \u2022 Characters distributed proportionally</span>
        </div>
      ...
      `,s.appendChild(l);let d=l.querySelector(".master-length-slider"),p=l.querySelector(".current-length"),u=l.querySelector(".btn-regenerate-thread"),y=l.querySelectorAll(".preset-btn");d.addEventListener("input",g=>{p.textContent=g.target.value}),y.forEach(g=>{g.addEventListener("click",()=>{let v=g.dataset.length;d.value=v,p.textContent=v})}),u.addEventListener("click",async()=>{let g=parseInt(d.value);await this.regenerateEntireThreadForGenerator(s,t,g,n,a,i)}),e.forEach((g,v)=>{let w=`Thread ${v+1}/${e.length}`,f=this.createTwitterCard(g,w,!0);f.dataset.platform="thread",f.dataset.threadId=t,f.dataset.category=n,s.appendChild(f)}),this.messagesContainer.appendChild(s),setTimeout(()=>{this.messagesContainer.scrollTo({top:this.messagesContainer.scrollHeight,behavior:"smooth"})},100)},regenerateEntireThreadForGenerator:async function(e,t,n,a,i,s){let o=e.querySelector(".btn-regenerate-thread");if(!o)return;let r=o.textContent;o.textContent="\u23F3 Regenerating...",o.disabled=!0;try{let c=Math.max(3,Math.min(12,Math.ceil(n/400))),m="";if(s){let u=await this.loadKnowledgePack(a);u&&u.facts&&(m=`

RELEVANT KNOWLEDGE BASE:
${u.facts.slice(0,5).map((y,g)=>`${g+1}. ${y}`).join(`
`)}
`)}let l=`You are a masterful Twitter/X thread storyteller crafting ${c} tweets (${n} total characters) about ${a}. Each tweet vibrates with personality, energy, and human warmth. Write in plain text with strategic emojis - no hashtags, no URLs, no formatting. Pure storytelling magic.`,d=`Create a magnetic Twitter thread with EXACTLY ${c} tweets totaling approximately ${n} characters about: ${i}
...
- Maintain factual accuracy from both tones`},renderSavedCustomTones:function(){let e=document.getElementById("saved-custom-tones");if(!e)return;if(this.customTones.length===0){e.classList.add("hidden");return}e.classList.remove("hidden"),e.innerHTML=`
      ...
      `,e.querySelectorAll(".saved-custom-tone").forEach(a=>{a.addEventListener("click",i=>{i.target.classList.contains("saved-tone-delete")||this.selectSavedCustomTone(a.dataset.customId)})}),e.querySelectorAll(".saved-tone-delete").forEach(a=>{a.addEventListener("click",i=>{i.stopPropagation(),this.deleteCustomTone(a.dataset.customId)})})},selectSavedCustomTone:function(e){let t=this.customTones.find(s=>s.id===e);if(!t)return;let n=this.toneDefinitions[t.tone1Id],a=this.toneDefinitions[t.tone2Id];this.selectedToneId="custom",this.selectedTone={id:"custom",name:t.name,tone1:n,tone2:a,aiInstructions:this.combineAIInstructions(n,a)};let i=document.getElementById("tone-generate-btn");i&&(i.disabled=!1),this.showToast("\u2713 Custom tone selected!")},deleteCustomTone:async function(e){this.customTones=this.customTones.filter(t=>t.id!==e),await this.saveCustomTones(),this.renderSavedCustomTones(),this.showToast("Custom tone deleted")},handleGenerate:function(){this.selectedTone&&(this.onGenerateCallback&&this.onGenerateCallback(this.selectedTone,this.currentPlatform),this.hideModal())},resetSelections:function(){document.querySelectorAll(".tone-option").forEach(n=>{n.classList.remove("selected"),n.setAttribute("aria-checked","false")}),this.selectedToneId=null,this.selectedTone=null;let t=document.getElementById("tone-generate-btn");t&&(t.disabled=!0)},showToast:function(e){let t=document.createElement("div");t.className="tone-toast",t.textContent=e,t.style.cssText=`
        ...
        z-index: 10001;
        animation: slideInUp 0.3s ease;
      `,document.body.appendChild(t),setTimeout(()=>{t.style.animation="slideOutDown 0.3s ease",setTimeout(()=>t.remove(),300)},2e3)},show:function(e,t,n){this.onGenerateCallback=n,this.showModal(e,t)}};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>h.init()):h.init(),window.TabTalkToneSelector=h})();(function(){let h={createUniversalCard:function(e,t={}){let{type:n="generic",title:a="Content",emoji:i="\u{1F4C4}",editable:s=n==="twitter"||n==="thread",showCharCount:o=n==="twitter"||n==="thread",showLengthControl:r=n==="twitter"||n==="thread",markdown:c=n!=="twitter"&&n!=="thread",maxCharLimit:m=n==="twitter"?280:n==="thread"?4e3:1200,originalData:l=null}=t,d=document.createElement("div");d.className="universal-card",d.dataset.cardType=n,l&&(d.dataset.originalContent=JSON.stringify(l));let p=document.createElement("div");p.className="universal-card-header";let u=document.createElement("span");u.className="universal-card-title",u.innerHTML=`${i} ${a}`;let y=document.createElement("div");y.className="actions-container";let g=document.createElement("button");g.className="copy-button",g.title="Copy content",g.textContent="\u{1F4CB}",y.appendChild(g),p.appendChild(u),p.appendChild(y);let v=document.createElement("div");v.className="universal-card-content";let w;if(s){w=document.createElement("textarea"),w.className="universal-text-area",w.placeholder=`Edit your ${n} content...`,w.value=e;let f=()=>{w.style.height="auto",w.style.height=Math.max(80,w.scrollHeight)+"px"};setTimeout(f,0),w.addEventListener("input",f)}else w=document.createElement("div"),w.className="universal-content-display",c&&window.marked?w.innerHTML=window.marked.parse(e):w.textContent=e;if(v.appendChild(w),o||r){let f=document.createElement("div");if(f.className="universal-controls",r){let T=document.createElement("div");T.className="length-control";let C=document.createElement("label");C.className="length-label",C.textContent="Target Length:";let b=document.createElement("input");b.type="range",b.className="length-slider";let E=n==="twitter"?40:n==="thread"?200:100,x=Math.max(E,m);b.min=String(E),b.max=String(x),b.step="10";let A=Math.min(m,Math.max(E,this.getAccurateCharacterCount(e,n)));b.value=String(A);let I=document.createElement("span");I.className="length-display",I.textContent=b.value;let k=document.createElement("button");k.className="regenerate-btn",k.title="Regenerate with new length",k.textContent="\u{1F504}",T.appendChild(C),T.appendChild(b),T.appendChild(I),T.appendChild(k),f.appendChild(T),b.addEventListener("input",()=>{I.textContent=b.value;let S=parseInt(b.value,10);this.updateProgressMetrics(d,S,n,m)}),k.addEventListener("click",async()=>{let S=parseInt(b.value);await this.handleRegenerate(d,S,n,l,m)})}if(o){let T=document.createElement("div");T.className="char-count",T.textContent=`${this.getAccurateCharacterCount(e,n)} characters`,f.appendChild(T);let C=document.createElement("div");C.className="char-progress-bar";let b=document.createElement("div");b.className="char-progress-fill",C.appendChild(b),f.appendChild(C),s&&w&&w.addEventListener("input",()=>{let E=this.getAccurateCharacterCount(w.value,n);T.textContent=`${E} characters`,this.updateProgressMetrics(d,E,n,m)})}v.appendChild(f),this.updateProgressMetrics(d,this.getAccurateCharacterCount(e,n),n,m)}if(d.appendChild(p),d.appendChild(v),g.addEventListener("click",()=>{let f=s?w.value:w.textContent;navigator.clipboard.writeText(f).then(()=>{g.textContent="\u2705",g.title="Copied!",setTimeout(()=>{g.textContent="\u{1F4CB}",g.title="Copy content"},2e3)}).catch(T=>{console.error("Failed to copy content:",T)})}),window.TabTalkUI&&window.TabTalkUI.addSaveButtonToCard){let f={id:Date.now().toString(),content:e,title:a,type:n};window.TabTalkUI.addSaveButtonToCard(y,n,f)}return d},getAccurateCharacterCount:function(e,t="generic"){if(typeof window.calculatePlatformCharacterCount=="function"){let n=t==="thread"?"twitter":t;return window.calculatePlatformCharacterCount(e,n)}return(e||"").length},updateProgressMetrics:function(e,t,n,a=280){if(!e)return;let i=e.querySelector(".char-count"),s=e.querySelector(".char-progress-fill"),o=e.querySelector(".length-slider"),r=n==="thread"?a:Math.min(a,n==="twitter"?280:a),c=o?parseInt(o.value,10):r,m=n==="twitter"?280:n==="thread"?a:r,l=Math.max(0,Math.min(100,t/m*100));if(s&&(s.style.width=`${l}%`,l<70?s.dataset.zone="safe":l<90?s.dataset.zone="warning":s.dataset.zone="danger"),i&&(i.textContent=`${t} / ${m} characters`,i.dataset.zone=l>=100?"danger":l>=90?"warning":"safe"),o){let d=Math.max(0,Math.min(100,c/m*100));o.dataset.targetPercent=String(d)}},handleRegenerate:async function(e,t,n,a,i=280){if(!window.TabTalkAI||!a){console.warn("Cannot regenerate: missing TabTalkAI instance or original data");return}try{let s=e.querySelector(".regenerate-btn"),o=s.textContent;s.textContent="\u23F3",s.disabled=!0,n==="twitter"||n==="thread"?await window.TabTalkAI.regenerateWithLength(e,t,n,i):window.TabTalkAI?.regenerateAnalyticsWithLength&&await window.TabTalkAI.regenerateAnalyticsWithLength(e,t,n,i),s.textContent=o,s.disabled=!1}catch(s){console.error("Regeneration failed:",s);let o=e.querySelector(".regenerate-btn");o.textContent="\u{1F504}",o.disabled=!1}},createTwitterCard:function(e,t,n=!1){return this.createUniversalCard(e,{type:n?"thread":"twitter",title:t,emoji:n?"\u{1F9F5}":"\u{1F426}",editable:!0,showCharCount:!0,showLengthControl:!0,markdown:!1,maxCharLimit:n?4e3:280,originalData:{content:window.TabTalkAI?.pageContent||e,platform:n?"thread":"twitter"}})},createAnalyticsCard:function(e,t,n){let a={summary:"\u{1F4DD}",keypoints:"\u{1F511}",analysis:"\u{1F4CA}",faq:"\u2753",factcheck:"\u2705",blog:"\u{1F4F0}",proscons:"\u2696\uFE0F",timeline:"\u{1F4C5}",quotes:"\u{1F4AC}"},i={summary:400,keypoints:600,analysis:1200,faq:1200,factcheck:1500,blog:1800,proscons:800,timeline:1e3,quotes:600};return this.createUniversalCard(e,{type:t,title:n||t.charAt(0).toUpperCase()+t.slice(1),emoji:a[t]||"\u{1F4C4}",editable:!1,showCharCount:!0,showLengthControl:!0,markdown:!0,maxCharLimit:i[t]||1200,originalData:{content:window.TabTalkAI?.pageContent||e,type:t}})}};window.UniversalCards=h})();(function(){let h={STORAGE_KEY:"niche_system",currentNiche:null,async init(){await this.loadNicheData(),this.bindEvents()},async loadNicheData(){try{let e=await this.getStorageItem(this.STORAGE_KEY);e&&(this.currentNiche=e.currentNiche,console.log("Niche system loaded:",this.currentNiche))}catch(e){console.error("Failed to load niche data:",e)}},async saveNicheData(){try{let e={currentNiche:this.currentNiche,lastUpdated:Date.now()};await this.setStorageItem(this.STORAGE_KEY,e)}catch(e){console.error("Failed to save niche data:",e)}},async setNiche(e){if(!e||e.trim().length===0)throw new Error("Niche description cannot be empty");return this.currentNiche={description:e.trim(),createdAt:Date.now(),id:this.generateId()},await this.saveNicheData(),console.log("Niche set to:",this.currentNiche),this.currentNiche},getCurrentNiche(){return this.currentNiche},hasNiche(){return this.currentNiche!==null},async clearNiche(){this.currentNiche=null,await this.saveNicheData(),console.log("Niche cleared")},generateId(){return"niche_"+Date.now()+"_"+Math.random().toString(36).substr(2,9)},async getStorageItem(e){try{return(await chrome.storage.local.get([e]))[e]}catch(t){return console.error("getStorageItem error:",t),null}},async setStorageItem(e,t){try{return await chrome.storage.local.set({[e]:t}),!0}catch(n){return console.error("setStorageItem error:",n),!1}},bindEvents(){}};window.NicheManager=h})();(function(){let h={STORAGE_KEY:"ai_memory",MAX_ITEMS_PER_NICHE:50,SUMMARY_MAX_LENGTH:500,async init(){await this.cleanupStorage()},async addMemoryItem(e,t,n={}){try{if(!window.NicheManager.hasNiche())throw new Error("Please set a niche first");let a=window.NicheManager.getCurrentNiche().id,i=await this.getMemoryData();if(i.niches[a]||(i.niches[a]=[]),i.niches[a].length>=this.MAX_ITEMS_PER_NICHE){let o=i.niches[a].filter(r=>!r.isViral);if(o.length>0){let r=o[0],c=i.niches[a].indexOf(r);i.niches[a].splice(c,1)}else throw new Error("Memory limit reached. Please mark some items as non-viral or upgrade for more storage.")}let s={id:this.generateId(),type:e,content:t,summary:await this.summarizeContent(t),metadata:n,isViral:!1,createdAt:Date.now(),nicheId:a};return i.niches[a].unshift(s),await this.saveMemoryData(i),console.log("Memory item added:",s.id),s}catch(a){throw console.error("Failed to add memory item:",a),a}},async markAsViral(e){let t=await this.getMemoryData(),n=window.NicheManager.getCurrentNiche().id;if(t.niches[n]){let a=t.niches[n].find(i=>i.id===e);if(a)return a.isViral=!0,a.markedViralAt=Date.now(),await this.saveMemoryData(t),console.log("Item marked as viral:",e),!0}return!1},async removeMemoryItem(e){let t=await this.getMemoryData(),n=window.NicheManager.getCurrentNiche().id;if(t.niches[n]){let a=t.niches[n].findIndex(i=>i.id===e);if(a!==-1)return t.niches[n].splice(a,1),await this.saveMemoryData(t),console.log("Memory item removed:",e),!0}return!1},async getMemoryItems(){if(!window.NicheManager.hasNiche())return[];let e=window.NicheManager.getCurrentNiche().id;return(await this.getMemoryData()).niches[e]||[]},async getSmartContext(e=10){let t=await this.getMemoryItems();return t.length===0?"":`Previous successful content in this niche:
